...
static unsigned int cramfs_nand_good_offset(struct super_block *sb, unsigned int offset)
{
...
//	printk("[%s:%d] offset = %x %x\n",__func__,__LINE__,
//		offset,(offset + nandinfo->block_map[offset >> nandinfo->erasesize_shift]));

	return  (offset + nandinfo->block_map[offset >> nandinfo->erasesize_shift]);
}

static uint32_t cramfs_next_good_block(struct mtd_info *mtd,uint32_t block_offset)
{
	while (1) {
		loff_t offs;
		...
		printk("[%s:%d]Skipping bad block at 0x%08x\n", __func__,__LINE__,block_offset);
...
//	printk("[%s:%d] PAGE_CACHE_SIZE = %ld\n",__func__,__LINE__,PAGE_CACHE_SIZE);
	...
	if(MAJOR(sb->s_dev) == MTD_BLOCK_MAJOR){
		...
		if(mtd->type != MTD_NANDFLASH)
			return;
//		printk("[%s:%d] mtd->size = %llx\n",__func__,__LINE__,mtd->size);
...
//		printk("[%s:%d] blocks = %d\n",__func__,__LINE__,blocks);
		for(i = 0; i < blocks; i++) {
			...
			newoffs = cramfs_next_good_block(mtd, offs);
			if(newoffs >= mtd->size)
				newoffs = offs;
			offset += newoffs - offs;
			block_map[i] = offset;
//			printk("[%s:%d]block_map[%02d] = %08x %08x[%x]\n",__func__,__LINE__,
//				i,i*mtd->erasesize,i*mtd->erasesize+block_map[i],block_map[i]);
		}
...
static void *cramfs_read(struct super_block *sb, unsigned int offset, unsigned int len)
{
	...
	offset = cramfs_nand_good_offset(sb, offset);	
#endif
...
#ifdef CONFIG_CRAMFS_NAND
	struct cramfs_nand_info *nandinfo = (struct cramfs_nand_info *)(sbi + 1);
	if(nandinfo)
