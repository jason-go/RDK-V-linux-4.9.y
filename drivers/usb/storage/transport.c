...
#if defined(CONFIG_SIRIUS) && defined(CONFIG_CPU_CACHE_DISABLE_READ_ALLOCATE_MODE)
#define MEM_ACCESS_4BYTE_ALIGN
#endif

#ifdef MEM_ACCESS_4BYTE_ALIGN
static void write_in_word(unsigned int *dst, unsigned char *src, int len)
{
	int times = len / 4;
	int left  = len % 4;
	unsigned int value, before_value;
	int i;
	...
	case 1:
		before_value = *(dst + i);
		value = (before_value & 0xffffff00) | (src[4*i + 0] << 0);
		*(dst + i) = value;
		break;
	case 2:
		before_value = *(dst + i);
		value = (before_value & 0xffff0000) | (src[4*i + 0] << 0) | (src[4*i + 1] << 8);
		*(dst + i) = value;
		break;
	case 3:
		before_value = *(dst + i);
		value = (before_value & 0xff000000) | (src[4*i + 0] << 0) | (src[4*i + 1] << 8) | (src[4*i + 2] << 16);
		*(dst + i) = value;
		break;
	default:
		break;
...
int usb_stor_Bulk_transport(struct scsi_cmnd *srb, struct us_data *us)
{
...
#ifdef MEM_ACCESS_4BYTE_ALIGN
	unsigned char flags, lun, length;
#endif
	...
	flags  = srb->sc_data_direction == DMA_FROM_DEVICE ? US_BULK_FLAG_IN : 0;
	lun    = srb->device->lun;
	length = srb->cmd_len;
	if (length > 0) {
		*((unsigned int *)(&bcb->Flags)) = (flags << 0) | (lun << 8) | (length << 16) | (srb->cmnd[0] << 24);
		write_in_word((unsigned int *)&bcb->CDB[1], &srb->cmnd[1], bcb->Length - 1);
	} else
		*((unsigned int *)(&bcb->Flags)) = (flags << 0) | (lun << 8) | (length << 16) | ((unsigned char)0 << 24);
#else
	/* set up the command wrapper */
	bcb->Signature = cpu_to_le32(US_BULK_CB_SIGN);
	bcb->DataTransferLength = cpu_to_le32(transfer_length);
