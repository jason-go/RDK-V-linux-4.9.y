		...
		struct nand_bbt_descr *td, int offs)
{
	...
	while (totlen) {
		...
		res = mtd->_read(mtd, from, len, &retlen, buf);
		if (res < 0) {
			if (retlen != len) {
			 ...
			 size_t len)
{
	...
	while (len > 0) {
		if (len <= mtd->writesize) {
			...
			ops.datbuf = buf;
			ops.len = len;
			return mtd->_read_oob(mtd, offs, &ops);
		} else {
			   ...
			   loff_t offs, uint8_t *buf, int len)
{
	...
	for (j = 0; j < len; j++) {
		...
		if (ret)
			return 1/*ret*/;/* read page ecc error,must be bad block*/
...
//	printk(KERN_INFO "[%s:%d]readlen=%d numblocks=%d startblock=%d from=%d scanlen=%d\n",
//		__func__,__LINE__,readlen,numblocks,startblock,from,scanlen);
	...
	for (i = startblock; i < numblocks;) {
		...
		if (ret) {
			...
			printk(KERN_WARNING "\033[0;32;32m""Bad eraseblock %05d at 0x%08llx ~ 0x%08llx""\033[m""\n",
			       i >> 1, (unsigned long long)from,(unsigned long long)(from + (1 << this->bbt_erase_shift)));
			mtd->ecc_stats.badblocks++;
		}

		i += 2;/*numblocks must  x2 */
		     ...
		     int chipsel)
{
	...
	write:
		...
		if (td->options & NAND_BBT_SAVECONTENT) {
			...
			res = mtd->_read(mtd, to, len, &retlen, buf);
			if (res < 0) {
				if (retlen != len) {
			...
			ops.oobbuf = &buf[len];
			res = mtd->_read_oob(mtd, to + mtd->writesize, &ops);
 ...
 *
*/
static int spinand_scan_bbt(struct mtd_info *mtd, struct nand_bbt_descr *bd)
{
	struct spinand_chip *this = mtd->priv;
	int len, res = 0;
	uint8_t *buf;
...
#define BBT_SCAN_OPTIONS (NAND_BBT_CREATE | NAND_BBT_WRITE \
...
static int spinand_create_default_bbt_descr(struct spinand_chip *this)
{
	...
	bd->len = (this->options & NAND_BBT_2BIT/*NAND_BUSWIDTH_16*/) ? 2 : 1;
...
int spinand_default_bbt(struct mtd_info *mtd)
{
	...
	if (this->options & NAND_IS_AND) {
		...
		this->options |= NAND_USE_FLASH_BBT;
		return spinand_scan_bbt(mtd, &agand_flashbased);
	}
...
int spinand_isbad_bbt(struct mtd_info *mtd, loff_t offs, int allowbbt)
{
	...
	case 0x00:
		return 0;
	case 0x01:
//		printk(KERN_INFO "\033[0;32;32m""bbt info for offs 0x%08x: (block %d) 0x%02x - BAD""\033[m""\n",
//	      		(unsigned int)offs, block >> 1, res);	
		return 1;
	case 0x02:
		return allowbbt ? 0 : 1;
