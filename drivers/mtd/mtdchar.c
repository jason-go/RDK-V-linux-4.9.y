...
static int mtdchar_ioctl(struct file *file, u_int cmd, u_long arg)
{
	...
	case MEMGETTOTALINFO:
		{
			struct mtd_total_info_user total_info;
			struct mtd_part *part = PART(mtd);
			memset(&total_info, 0, sizeof(total_info));
			total_info.type		= mtd->type;
			total_info.flags	= mtd->flags;
			total_info.offset	= part->offset;
			total_info.size		= mtd->size;
			total_info.erasesize	= mtd->erasesize;
			total_info.writesize	= mtd->writesize;
			total_info.oobsize	= mtd->oobsize;
			/* The below fields are obsolete */
			total_info.ecctype	= -1;
			if (copy_to_user(argp, &total_info, sizeof(struct mtd_total_info_user)))
				return -EFAULT;
		}
		break;
	case MEMRESIZE:
		{
			struct mtd_part *part = PART(mtd);
			struct mtd_resize buf;
			/* NOTE: writes return length to buf_user->length */
			if (copy_from_user(&buf, argp, sizeof(buf)))
				ret = -EFAULT;
			else {
				printk("Before: mtd->size = %llx offset - %llx\n",mtd->size,part->offset);
				mtd->size = buf.size;
				part->offset = buf.start;
				printk("After: mtd->size = %llx offset - %llx\n",mtd->size,part->offset);
			}
		}
		break;
	case MEMERASE:
