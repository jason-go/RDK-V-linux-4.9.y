...
void usbnet_cdc_unbind(struct usbnet *dev, struct usb_interface *intf)
{
	struct cdc_state		*info = (void *) &dev->data;
	struct usb_driver		*driver = driver_of(intf);
	struct cdc_data *cdc_data = (struct cdc_data *)dev->driver_priv;
	...
	if (cdc_data) {
		flush_workqueue(cdc_data->ax_work);
		destroy_workqueue(cdc_data->ax_work);
		kfree(cdc_data);
...
void usbnet_cdc_status(struct usbnet *dev, struct urb *urb)
{
	...
	case USB_CDC_NOTIFY_NETWORK_CONNECTION:
		...
		if (event->wValue)
			cdc_data->event = PHY_POWER_UP;
		else
			cdc_data->event = PHY_POWER_DOWN;
		if(netif_carrier_ok(dev->net) != cdc_data->link) {
			cdc_data->link = cdc_data->event;
			queue_work(cdc_data->ax_work, &cdc_data->check_link);
		}
		break;
...
static void cdc_link_reset(struct work_struct *work)
{
	struct cdc_data *cdc_data = container_of(work,
						       struct cdc_data,
						       check_link);
	struct usbnet *dev = cdc_data->dev;

	switch (cdc_data->event) {
	case PHY_POWER_DOWN:
		kobject_uevent(&dev->net->dev.kobj, KOBJ_REMOVE);
		break;
	case PHY_POWER_UP:
		kobject_uevent(&dev->net->dev.kobj, KOBJ_ADD);
		break;
	default:
		break;
	}
...
int usbnet_cdc_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	cdc_data = kmalloc(sizeof(*cdc_data), GFP_KERNEL);
	if (!cdc_data) {
		netdev_err(dev->net, "Cannot allocate memory for cdc data");
		return -ENOMEM;
	}

	memset(cdc_data, 0, sizeof(*cdc_data));
	dev->driver_priv = cdc_data;

	cdc_data->ax_work = create_singlethread_workqueue("cdc");
	if (!cdc_data->ax_work) {
		kfree(cdc_data);
		return -ENOMEM;
	}
	...
	INIT_WORK(&cdc_data->check_link, cdc_link_reset);
