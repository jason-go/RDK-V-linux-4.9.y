			 ...
			 void __iomem **ioremaped_shm)
{
	...
	if (!va) {
		dev_err(dev, "shared memory ioremap failed, addr = %d, size = %d\n", paddr, size);
		return ERR_PTR(-EINVAL);
	}
...
asmlinkage void optee_arm_smccc_smc(unsigned long a0, unsigned long a1,
...
asmlinkage void optee_arm_smccc_hvc(unsigned long a0, unsigned long a1,
			unsigned long a2, unsigned long a3, unsigned long a4,
			unsigned long a5, unsigned long a6, unsigned long a7,
			struct arm_smccc_res *res)
{
	__arm_smccc_hvc(a0, a1, a2, a3, a4, a5, a6, a7, res, NULL);
}
...
static int get_invoke_func(struct device *dev, optee_invoke_fn **invoke_fn)
{
	...
	if (!strcmp("hvc", method)) {
		*invoke_fn = optee_arm_smccc_hvc;
	} else if (!strcmp("smc", method)) {
		*invoke_fn = optee_arm_smccc_smc;
	} else {
