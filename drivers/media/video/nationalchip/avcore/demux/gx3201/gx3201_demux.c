 ...
 ******************************************************************************
 File Name     : gx3201_demux.c
Version       : Initial Draft
Author        : Nationalchip multimedia software group
Created       : 2008/12/18
Last Modified :
Description   : gx3201 chip demux module outer interface
Function List :
History       :
1.Date        : 2008/12/18
Author      : zz
...
struct dmxdev gx3201_dmx;
struct reg_demux *gx3201_demux_reg = NULL;

#define DEMUX_THREAD_STACK (16 * 1024)
static unsigned char *thread_stack;
static gx_mutex_t demux_mutex;

static int gx3201_demux_init(struct gxav_device *device, struct gxav_module_inode *inode)
{
	int ret = 0;
	...
	gx3201_demux_reg = gx_ioremap(GXAV_DEMUX_BASE_SYS, sizeof(struct reg_demux));
	if (!gx3201_demux_reg) {
		DMX_PRINTF("ioremap DEMUX space failed!\n");
		return -1;
	}

	DMX_DBG("gx3201_demux_reg = 0x%p\n",gx3201_demux_reg);

	gx3201_dmx_initialization(&gx3201_dmx);
	REG_CLR_BIT(&gx3201_demux_reg->pts_mode, 0);
	REG_SET_VAL(&gx3201_demux_reg->df_buf_full_stop_th, 0);
	REG_SET_VAL(&gx3201_demux_reg->av_buf_full_stop_th, 0);

	ret = gx_sem_create(&(gx3201_dmx.sem_id), 0);
	if (ret < 0) {
	...
	ret = gx_thread_create("gx3201_demux_thread", &(gx3201_dmx.thread_id), gx3201_demux_read_thread,
			NULL, thread_stack, DEMUX_THREAD_STACK, 9, &(gx3201_dmx.thread_info));
	if (ret < 0) {
		DMX_PRINTF("demux thread created fail!\n");
		return -1;
	}

	gx_spin_lock_init(&(gx3201_dmx.demux_spin_lock));
	gx_mutex_init(&(demux_mutex));
	return 0;
}

static int gx3201_demux_cleanup(struct gxav_device *device, struct gxav_module_inode *inode)
{
	int i, j;
	...
	if (0 != gx3201_dmx.ca_mask) {
		for (i = 0; i < MAX_CA_NUM; i++) {
			if (NULL != gx3201_dmx.ca[i]) {
				gx3201_dmx_free_ca(gx3201_dmx.ca[i]);
			}
		}
	}

	gx_iounmap(gx3201_demux_reg);
	gx3201_demux_reg = NULL;
	gx_release_mem_region(GXAV_DEMUX_BASE_SYS, sizeof(struct reg_demux));

	gx_sem_delete(&(gx3201_dmx.sem_id));
	gx_memset(&(gx3201_dmx.sem_id),0,sizeof(gx_sem_id));

	gx_thread_delete(gx3201_dmx.thread_id);
	if(thread_stack !=NULL){
		gx_free(thread_stack);
		thread_stack = NULL;
	}
	gx3201_dmx_cleanup(&gx3201_dmx);
	gx_mutex_destroy(&(demux_mutex));

	return 0;
}

static int gx3201_demux_open(struct gxav_module *module)
{
	struct dmx_demux *demux = NULL;
	...
	demux = &gx3201_dmx.demuxs[module->sub];
	demux->demux_device = &gx3201_dmx;
	demux->id = module->sub;
...
static void gx3201_demux_clr_ints(void)
{
	int i;
	for(i=gx3201_demux_reg->int_ts_if; i<= gx3201_demux_reg->int_dmx_cc_en_l; i+=4){
		*(volatile int*)i = 0xffffffff;
	}
}

static int gx3201_demux_close(struct gxav_module *module)
{
	int i;
	struct dmx_demux *demux = NULL;
	...
	if (0 != demux->slot_num) {
		for (i = 0; i < MAX_SLOT_NUM; i++) {
			if (NULL != demux->slots[i]) {
				gx3201_dmx_free_slot(demux->slots[i]);
				if (0 == demux->slot_num) {
	...
	if (0 != demux->muxslot_num) {
		for (i = 0; i < MAX_SLOT_NUM; i++) {
			if (NULL != demux->muxslots[i]) {
				gx3201_dmx_free_slot(demux->muxslots[i]);
				if (0 == demux->muxslot_num) {
					DMX_DBG("slot free over!\n");
					break;
				}
	...
	for (i = 0; i < MAX_CA_NUM; i++) {
		if (NULL != demux->ca[i]) {
			gx3201_dmx_free_ca(demux->ca[i]);
		}
	}
	gx3201_demux_clr_ints();

	return 0;
}

static int gx3201_demux_config(int sub,GxDemuxProperty_ConfigDemux *config_dmx)
{
	struct dmx_demux *demux = NULL;
	...
	gx3201_dmx_config(demux);

	return 0;
}

static int gx3201_demux_lock(int sub,GxDemuxProperty_TSLockQuery *query_tslock)
{
	struct dmx_demux *demux = NULL;
	...
	demux = &(gx3201_dmx.demuxs[sub]);
	gx3201_dmx_query_tslock(demux);
	query_tslock->ts_lock = demux->ts_lock;

	return 0;
}

int gx3201_dmx_query_dmx_slot_fifo(unsigned int buf_id, int slot_type, unsigned int *dmx_id, struct dmx_slot *slot, struct demux_fifo *fifo)
{
	unsigned int i = 0;

	for(i=0; i<MAX_AVBUF_NUM; i++) {
		if(gx3201_dmx.avfifo[i].channel_id == buf_id && gx3201_dmx.avfifo[i].channel != NULL) {
			gx_memcpy(fifo, &gx3201_dmx.avfifo[i], sizeof(struct demux_fifo));
			break;
		}
	}
	...
	if(gx3201_dmx.slots[fifo->pin_id] && gx3201_dmx.slots[fifo->pin_id]->type == slot_type)
		*dmx_id = gx3201_dmx.slots[fifo->pin_id]->demux->id;
	else
		return -1;

	*slot = *gx3201_dmx.slots[fifo->pin_id];

	return 0;
}

int gx3201_demux_link_fifo(int sub, struct demux_fifo *fifo)
{
	struct dmx_demux *demux = NULL;
	...
	if (fifo->pin_id < 32) {
		if (!(gx3201_dmx.slots[fifo->pin_id])
				|| (gx3201_dmx.slots[fifo->pin_id]->type < DEMUX_SLOT_AUDIO)) {
			DMX_PRINTF("pin_id error!\n");
			return -1;
		}
	}

	return gx3201_dmx_link_fifo(demux, fifo);
}

int gx3201_demux_unlink_fifo(int sub, struct demux_fifo *fifo)
{
	struct dmx_demux *demux = NULL;
	...
	return gx3201_dmx_unlink_fifo(demux, fifo);
}

static int gx3201_demux_pcr(int sub,GxDemuxProperty_Pcr *pcr)
{
	if (NULL == pcr)
		return -1;

	gx3201_dmx_pcr_sync(&gx3201_dmx, sub, pcr->pcr_pid);
	return 0;
}

static int gx3201_demux_stc(int sub,GxDemuxProperty_ReadStc *stc)
{
#ifndef NO_OS
	if (NULL == stc)
		return -1;
...
static int gx3201_demux_slot_allocate(int sub,GxDemuxProperty_Slot *alloc_slot)
{
	...
	if ((alloc_slot->type < DEMUX_SLOT_PSI) || (alloc_slot->type > DEMUX_SLOT_VIDEO)) {
		DMX_PRINTF("gx3201 Allocate: the slot's type error!\n");
		return -1;
	}

	slot_type = alloc_slot->type;
	pid = alloc_slot->pid;
	alloc_slot->slot_id = gx3201_dmx_alloc_slot(&gx3201_dmx, sub, slot_type,pid);
...
static int gx3201_demux_slot_free(int sub,GxDemuxProperty_Slot *free_slot)
{
	...
	if(DEMUX_SLOT_MUXTS == free_slot->type){
		slot = gx3201_dmx.muxslots[(free_slot->slot_id - PIN_ID_SDRAM_OUTPUT)];
	}else{
		if(sub == 2)
			slot = gx3201_dmx.pidslots[free_slot->slot_id];
		else
			slot = gx3201_dmx.slots[free_slot->slot_id];
	}
	...
	DMX_DBG("slot->id = %d\n", slot->id);
	gx3201_dmx_free_slot(slot);

	return 0;
}

static int gx3201_demux_slot_config(int sub,GxDemuxProperty_Slot *config_slot)
{
	struct dmx_slot *slot = NULL;
	...
	if(sub == 2)
		slot = gx3201_dmx.pidslots[config_slot->slot_id];
	else
		slot = gx3201_dmx.slots[config_slot->slot_id];
	...
	gx3201_dmx_slot_config(slot);

	return 0;
}

static int gx3201_demux_slot_enable(int sub,GxDemuxProperty_Slot *enable_slot)
{
	struct dmx_slot *slot = NULL;
	...
	if(sub == 2)
		slot = gx3201_dmx.pidslots[enable_slot->slot_id];
	else
		slot = gx3201_dmx.slots[enable_slot->slot_id];
	...
	gx3201_dmx_enable_slot(slot);

	return 0;
}

static int gx3201_demux_slot_query(int sub,GxDemuxProperty_SlotQueryByPid *query_slot)
{
	int i;
	...
	for (i = 0; i < MAX_SLOT_NUM; i++)
	{
		if (NULL != gx3201_dmx.demuxs[sub].slots[i])
		{
			//挂载相同demux下的slot,因为不同的demux可以有相同的pid的不同slot。
			if (query_slot->pid == gx3201_dmx.demuxs[sub].slots[i]->pid
					&& sub == gx3201_dmx.demuxs[sub].slots[i]->demux->id)
			{
				query_slot->slot_id = gx3201_dmx.demuxs[sub].slots[i]->id;
				break;
			}
		}
	}
...
static int gx3201_demux_slot_disable(int sub,GxDemuxProperty_Slot *disable_slot)
{
	struct dmx_slot *slot = NULL;
	...
	if(sub == 2)
		slot = gx3201_dmx.pidslots[disable_slot->slot_id];
	else
		slot = gx3201_dmx.slots[disable_slot->slot_id];
	...
	if(slot->filter_num  < 1)
		gx3201_dmx_disable_slot(slot);

	return 0;
}

static int gx3201_demux_slot_attribute(int sub,GxDemuxProperty_Slot *config_slot)
{
	struct dmx_slot *slot = NULL;
...
static int gx3201_demux_filter_allocate(int sub,GxDemuxProperty_Filter *alloc_filter)
{
	struct dmx_slot *slot = NULL;
	...
	if(alloc_filter->slot_id >= PIN_ID_SDRAM_OUTPUT)
		slot = gx3201_dmx.muxslots[(alloc_filter->slot_id - PIN_ID_SDRAM_OUTPUT)];
	else
		slot = gx3201_dmx.slots[alloc_filter->slot_id];
	...
	alloc_filter->filter_id = gx3201_dmx_alloc_filter(slot);
...
static int gx3201_demux_filter_free(int sub,GxDemuxProperty_Filter *free_filter)
{
	...
	if(free_filter->filter_id >= PIN_ID_SDRAM_OUTPUT)
		filter = gx3201_dmx.muxfilters[(free_filter->filter_id - PIN_ID_SDRAM_OUTPUT)];
	else
		filter = gx3201_dmx.filters[free_filter->filter_id];
	...
	gx3201_dmx_free_filter(filter);

	return 0;
}

static int gx3201_demux_filter_fifo_reset(int sub,GxDemuxProperty_FilterFifoReset *reset_filter_fifo)
{
	struct dmx_filter *filter = NULL;
	unsigned int mask = 0;
	...
	if(reset_filter_fifo->filter_id >= PIN_ID_SDRAM_OUTPUT)
		filter = gx3201_dmx.muxfilters[(reset_filter_fifo->filter_id - PIN_ID_SDRAM_OUTPUT)];
	else
		filter = gx3201_dmx.filters[reset_filter_fifo->filter_id];
	...
	gx3201_set_filter_reset(filter);

	return 0;
}

static int gx3201_demux_filter_fifo_query(int sub,GxDemuxProperty_FilterFifoQuery *query_filter_fifo)
{
	if (NULL == query_filter_fifo)
		return -1;

	gx3201_dmx_query_fifo(&(query_filter_fifo->state));

	return 0;
}

static int gx3201_demux_filter_config(int sub,GxDemuxProperty_Filter *config_filter)
{
	...
	filter = gx3201_dmx.filters[config_filter->filter_id];
	if (NULL == filter) {
		DMX_PRINTF("FilterConfig: filter is NULL\n");
		return -1;
	}
    gx3201_clear_int_df(config_filter->filter_id);
	...
	gx3201_dmx_filter_config(filter);

	return 0;
}

static int gx3201_demux_filter_read(int sub,GxDemuxProperty_FilterRead *read_filter)
{
	struct dmx_filter *filter = NULL;
	...
	if(read_filter->filter_id >= PIN_ID_SDRAM_OUTPUT)
		filter = gx3201_dmx.muxfilters[(read_filter->filter_id - PIN_ID_SDRAM_OUTPUT)];
	else
		filter = gx3201_dmx.filters[read_filter->filter_id];

	if (NULL == filter) {
		DMX_PRINTF("FilterRead: the demux read filter NULL!\n");
		return -1;
	}
	read_filter->read_size = gx3201_dmx_filter_read(filter, (unsigned char *)read_filter->buffer,read_filter->max_size);

	return 0;
}

static int gx3201_demux_filter_enable(int sub,GxDemuxProperty_Filter *enable_filter)
{
	struct dmx_filter *filter = NULL;
	...
	filter = gx3201_dmx.filters[enable_filter->filter_id];
	if (NULL == filter) {
		DMX_PRINTF("FilterEnable: filter is NULL\n");
		return -1;
	}

	gx3201_dmx_enable_filter(filter);

	return 0;
}

static int gx3201_demux_filter_disable(int sub,GxDemuxProperty_Filter *disable_filter)
{
	struct dmx_filter *filter = NULL;
	...
	filter = gx3201_dmx.filters[disable_filter->filter_id];
	if (NULL == filter) {
		DMX_PRINTF("FilterDisable: filter is NULL\n");
		return -1;
	}

	gx3201_dmx_disable_filter(filter);

	return 0;
}

static int gx3201_demux_filter_attribute(int sub,GxDemuxProperty_Filter *config_filter)
{
	//int depth = 0;
	struct dmx_filter *filter = NULL;
...
static int gx3201_demux_filter_eventcb(int sub,unsigned int filter_id,filter_event_cb callback,void *priv)
{
	struct dmx_filter *filter = NULL;

	if(filter_id >= PIN_ID_SDRAM_OUTPUT)
		filter = gx3201_dmx.muxfilters[(filter_id - PIN_ID_SDRAM_OUTPUT)];
	else
		filter = gx3201_dmx.filters[filter_id];
...
static int gx3201_demux_ca_allocate(int sub,GxDemuxProperty_CA *alloc_ca)
{
	if (NULL == alloc_ca)
		return -1;

	alloc_ca->ca_id = gx3201_dmx_alloc_ca(&gx3201_dmx,sub);
	if (alloc_ca->ca_id < 0) {
...
static int gx3201_demux_ca_free(int sub,GxDemuxProperty_CA *config_ca)
{
	...
	ca = gx3201_dmx.ca[config_ca->ca_id];
	if (NULL == ca) {
		DMX_PRINTF("the ca config ca is NULL\n");
		return -1;
	}

	gx3201_dmx_free_ca(ca);

	return 0;
}

static int gx3201_demux_ca_config(int sub,GxDemuxProperty_CA *config_ca)
{
	struct dmx_slot *slot = NULL;
	struct dmx_ca *ca = NULL;
	...
	slot = gx3201_dmx.slots[config_ca->slot_id];
	ca = gx3201_dmx.ca[config_ca->ca_id];
	...
	gx3201_dmx_ca_config(slot, ca);

	return 0;
}

static int gx3201_demux_ca_attribute(int sub,GxDemuxProperty_CA *config_ca)
{
	struct dmx_ca *ca = NULL;
...
static int gx3201_demux_camtc_config(int sub,GxDemuxProperty_MTCCA *config_mtc)
{
	struct dmx_slot *slot = NULL;
	struct dmx_ca *ca = NULL;
	...
	slot = gx3201_dmx.slots[config_mtc->slot_id];
	ca = gx3201_dmx.ca[config_mtc->ca_id];
	if (NULL == ca) {
	...
	gx3201_dmx_mtc_decrypt(&mtc_info_cfg,ca);
	gx3201_set_mtc_descramble(ca,slot);

	return 0;
}

static int gx3201_demux_event_cb(unsigned int event,int ops,void *priv)
{
...
static struct gxav_module_inode* gx3201_demux_interrupt(struct gxav_module_inode *inode, int irq)
{
	if (irq != DMX_INTERRUPT_NUMBER || inode == NULL)
	{
		gx3201_demux_clr_ints();
		DMX_PRINTF("gx3201_demux_interrupt inode = NULL\n");
		return NULL;
	}
	...
	gx3201_dmx_interrupt(gx3201_demux_event_cb,inode);
	return inode;
}

static int gx3201_demux_fifo_free_size(GxDemuxProperty_FilterFifoFreeSize *fifo_free_size)
{
	struct dmx_filter *filter = NULL;
	...
	filter = gx3201_dmx.filters[fifo_free_size->filter_id];
	if (NULL == filter) {
...
extern int gx3201_demux_framebuffer_alloc(void);
extern int gx3201_demux_framebuffer_free(int i);
extern int gx3201_demux_query_fifo(void);

static int gx3201_demux_set_property(struct gxav_module *module, int property_id, void *property, int size)
{
	int ret = 0;
	if ((NULL == module) || (module->sub < 0) || (module->sub > module->inode->count))
		return -3;
		...
		case GxAVGenericPropertyID_ModuleUnLinkChannel:
			{
				...
				if (GxAVGenericPropertyID_ModuleLinkChannel == property_id) {
					ret = gx3201_demux_link_fifo(module->sub, &demux_fifo);
				} else
					ret = gx3201_demux_unlink_fifo(module->sub, &demux_fifo);
			}
			break;
		...
		case GxDemuxPropertyID_Config:
			{
				...
				if (size != sizeof(GxDemuxProperty_ConfigDemux)) {
					DMX_PRINTF("gx3201  ConfigDmx: the demux module's configure param error!\n");
					goto err;
				}

				ret = gx3201_demux_config(module->sub,config_dmx);
			}
			break;
		...
		case GxDemuxPropertyID_SlotConfig:
			{
				...
				ret = gx3201_demux_slot_config(module->sub,config_slot);
			}
			break;
		...
		case GxDemuxPropertyID_SlotEnable:
			{
				...
				ret = gx3201_demux_slot_enable(module->sub,enable_slot);
			}
			break;
		...
		case GxDemuxPropertyID_SlotDisable:
			{
				...
				ret = gx3201_demux_slot_disable(module->sub,disable_slot);
			}
			break;
		...
		case GxDemuxPropertyID_SlotFree:
			{
				...
				ret = gx3201_demux_slot_free(module->sub,free_slot);
			}
			break;
		...
		case GxDemuxPropertyID_FilterConfig:
			{
				...
				ret = gx3201_demux_filter_config(module->sub,config_filter);
			}
			break;
		...
		case GxDemuxPropertyID_FilterEnable:
			{
				...
				ret = gx3201_demux_filter_enable(module->sub,enable_filter);
			}
			break;
		...
		case GxDemuxPropertyID_FilterDisable:
			{
				...
				ret = gx3201_demux_filter_disable(module->sub,disable_filter);
			}
			break;
		...
		case GxDemuxPropertyID_FilterFIFOReset:
			{
				...
				ret = gx3201_demux_filter_fifo_reset(module->sub,reset_filter_fifo);
			}
			break;
		...
		case GxDemuxPropertyID_FilterFree:
			{
				...
				ret = gx3201_demux_filter_free(module->sub,free_filter);
			}
			break;
		...
		case GxDemuxPropertyID_CAConfig:
			{
				...
				ret = gx3201_demux_ca_config(module->sub, config_ca);
			}
			break;
		...
		case GxDemuxPropertyID_CAFree:
			{
				...
				ret = gx3201_demux_ca_free(module->sub, config_ca);
			}
			break;
		...
		case GxDemuxPropertyID_Pcr:
			{
				...
				ret = gx3201_demux_pcr(module->sub, pcr);
			}
			break;
		...
		case GxDemuxPropertyID_MTCCAConfig:
			{
				...
				ret = gx3201_demux_camtc_config(module->sub, config_mtc);
			}
			break;
...
static int gx3201_demux_get_property(struct gxav_module *module, int property_id, void *property, int size)
{
	int ret = 0;
	if ((NULL == module) || (module->sub < 0) || (module->sub > module->inode->count))
		return -3;
		...
		case GxDemuxPropertyID_TSLockQuery:
			{
				...
				if (size != sizeof(GxDemuxProperty_TSLockQuery)) {
					DMX_PRINTF("gx3201 QueryTsLock: the property error!\n");
					goto err;
				}

				ret = gx3201_demux_lock(module->sub, query_tslock);
			}
			break;
		...
		case GxDemuxPropertyID_SlotAlloc:
			{
				...
				if (size != sizeof(GxDemuxProperty_Slot)) {
					DMX_PRINTF("gx3201 AllocSlot: the demux slot's allocate param error!\n");
					goto err;
				}

				ret = gx3201_demux_slot_allocate(module->sub, alloc_slot);
			}
			break;
		...
		case GxDemuxPropertyID_SlotConfig:
			{
				...
				ret = gx3201_demux_slot_attribute(module->sub, config_slot);
			}
			break;
		...
		case GxDemuxPropertyID_FilterAlloc:
			{
				...
				ret = gx3201_demux_filter_allocate(module->sub, alloc_filter);
				if(ret != 0)
					goto err;

				ret = gx3201_demux_filter_eventcb(module->sub, alloc_filter->filter_id,
						gx3201_demux_event_cb, module->inode);
			}
			break;
		...
		case GxDemuxPropertyID_FilterConfig:
			{
				...
				ret = gx3201_demux_filter_attribute(module->sub, config_filter);
			}
			break;
		...
		case GxDemuxPropertyID_CAAlloc:
			{
				...
				ret = gx3201_demux_ca_allocate(module->sub, alloc_ca);
			}
			break;
		case GxDemuxPropertyID_CAConfig:
			{
				...
				ret = gx3201_demux_ca_attribute(module->sub, config_ca);
			}
			break;
		...
		case GxDemuxPropertyID_FilterRead:
			{
				...
				ret = gx3201_demux_filter_read(module->sub, read_filter);
			}
			break;
		...
		case GxDemuxPropertyID_FilterFIFOQuery:
			{
				...
				ret = gx3201_demux_filter_fifo_query(module->sub, query_filter_fifo);
			}
			break;
		...
		case GxDemuxPropertyID_SlotQueryByPid:
			{
				...
				ret = gx3201_demux_slot_query(module->sub, query_slot);
			}
			break;
		...
		case GxDemuxPropertyID_ReadStc:
			{
				...
				ret = gx3201_demux_stc(module->sub, stc);
			}
			break;
		...
		case GxDemuxPropertyID_FilterFifoFreeSize:
			{
				...
				ret = gx3201_demux_fifo_free_size(fifo_free_size);
			}
			break;
...
struct gxav_module_ops gx3201_demux_module = {
	.module_type  = GXAV_MOD_DEMUX,
	.count        = 3,
	.irqs         = {DMX_INTERRUPT_NUMBER, -1},
	.irq_flags    = {-1},
	.event_mask   = 0xffffffff,
	.open         = gx3201_demux_open,
	.close        = gx3201_demux_close,
	.init         = gx3201_demux_init,
	.cleanup      = gx3201_demux_cleanup,
	.set_property = gx3201_demux_set_property,
	.get_property = gx3201_demux_get_property,
	.interrupts[DMX_INTERRUPT_NUMBER]    = gx3201_demux_interrupt
};
