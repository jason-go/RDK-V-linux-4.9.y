...
#ifndef _GX3201_DEMUX_REGS_H
#define _GX3201_DEMUX_REGS_H
...
#define NUM_KEY_PAIR                       16    // gx3201...have 12 pair of keys
...
struct reg_demux {
	...
	unsigned int reserve_y[3];
	struct reg_demux_tsw_buf tsw_buf[64];
	unsigned int mtc_ctrl[2];
	unsigned int mtc_ca_config;
	unsigned int mtc_dck_config[6];
	unsigned int mtc_cw_config[4];
	unsigned int mtc_cw_addr_config;
	unsigned int reserve_z[2];
	struct reg_demux_dfaddr df_addr[64];
};


#endif /*_GX3201_DEMUX_REGS_H*/
