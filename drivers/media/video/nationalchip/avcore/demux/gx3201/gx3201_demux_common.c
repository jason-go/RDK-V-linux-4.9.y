...
void gx3201_demux_delay_free(struct dmxdev *dmxdev)
{
	...
	for(i=0;i<MAX_FILTER_NUM;i++){
		...
		if(filter != NULL && filter->status == DMX_STATE_DELAY_FREE){
			...
			if(time > 4000){
				...
				gx3201_clear_df_rw(id);
				gx3201_clear_df_buf(id);
...
int gx3201_dmx_avbuf_callback(unsigned int channel_id,unsigned int length, unsigned int underflow,void *arg)
{
	...
	avfifo = &gx3201_dmx.avfifo[av_bufid];
	gxav_sdc_rwaddr_get(avfifo->channel_id, &av_read_addr,&sdc_write_addr);
	//gx_printf("update=%08x %08x\n",av_write_addr,av_read_addr);
	REG_SET_VAL(&(gx3201_demux_reg->av_addr[av_bufid].av_read_addr), av_read_addr);
	av_write_addr = REG_GET_VAL(&(gx3201_demux_reg->av_addr[av_bufid].av_write_addr));
	if(av_write_addr != sdc_write_addr)
		gxav_sdc_Wptr_Set(avfifo->channel_id, av_write_addr);

	pts_write_addr = REG_GET_VAL(&(gx3201_demux_reg->pts_w_addr[av_bufid]));
	gxav_sdc_Wptr_Set(gx3201_dmx.avfifo[av_bufid].pts_channel_id, pts_write_addr);
	DMX_DBG("demux callback av_wptr 0x%x \n",av_write_addr);
	return 0;
}

void gx3201_dmx_initialization(struct dmxdev *demux_device)
{
...
void gx3201_dmx_cleanup(struct dmxdev *demux_device)
{
...
void gx3201_dmx_config(struct dmx_demux *dmx)
{
	gx3201_set_gate(dmx);
	gx3201_set_ts_source(dmx);
	gx3201_set_ts_mode(dmx);
}

int gx3201_dmx_link_fifo(struct dmx_demux *dmx, struct demux_fifo *fifo)
{
	...
	if (GXAV_PIN_INPUT == fifo->direction) {
		if (fifo->pin_id == PIN_ID_SDRAM_INPUT) {
			...
			gx3201_link_rts_sdc(start_addr, end_addr - start_addr + 1, buf_id);

			channel->indata = &dmx->demux_device->rfifo;
			channel->incallback = gx3201_rts_cb;
		}
	}
		...
		if ((fifo->pin_id >= PIN_ID_SDRAM_OUTPUT)) {
			...
			gx3201_link_wts_sdc(dmx,start_addr, end_addr - start_addr + 1, fifo->pin_id);

			channel->outdata = &dmx->demux_device->fifo[fifo->pin_id-PIN_ID_SDRAM_OUTPUT];
			channel->outcallback = gx3201_wts_cb;
		}
			...
			gx3201_link_avbuf(slot, start_addr, end_addr - start_addr + 1, buf_id);

			start_addr = fifo->pts_start_addr;
			end_addr   = fifo->pts_end_addr;
			buf_id     = fifo->pts_buffer_id;
			gx3201_link_ptsbuf(slot, start_addr, end_addr - start_addr + 1, buf_id);
			...
			channel->outcallback = gx3201_dmx_avbuf_callback;
		}
	}
...
int gx3201_dmx_unlink_fifo(struct dmx_demux *dmx, struct demux_fifo *fifo)
{
	if (GXAV_PIN_INPUT == fifo->direction) {
		struct gxav_channel *channel = (struct gxav_channel *)(fifo->channel);
		...
		if (fifo->pin_id == PIN_ID_SDRAM_OUTPUT /* fifo->pin_id >= PIN_ID_SDRAM_OUTPUT0 && fifo->pin_id <= PIN_ID_SDRAM_OUTPUT15 */ ) {
			...
			gx3201_unlink_wts_sdc(dmx,fifo->pin_id);
		} else if (fifo->pin_id < 64) {
			struct dmx_slot *slot = dmx->slots[fifo->pin_id];
			gx3201_unlink_avbuf(slot);
...
inline void gx3201_dmx_query_tslock(struct dmx_demux *dmx)
{
	gx3201_query_tslock(dmx);
}

int gx3201_dmx_alloc_slot(struct dmxdev *demux_device, int demux_id, enum dmx_slot_type type, unsigned short pid)
{
	int i;
	...
	if(DEMUX_SLOT_MUXTS == type){
		...
		if(pid == 0xffff)
		{
			int temp = REG_GET_VAL(&(gx3201_demux_reg->ts_write_ctrl));
	...
	if(demux_id == 2) {
		for (i = 0; i < MAX_SLOT_NUM; i++) {
			if (gx3201_dmx.pidslots[i] != NULL) {
				if ((gx3201_dmx.pidslots[i]->pid == pid) &&
						(gx3201_dmx.pidslots[i]->type == type) &&
						(pid != 0x1fff)) {
					gx3201_dmx.pidslots[i]->refcount++;
					return i;
				}
			}
		}
	...
	for (i = 0; i < MAX_SLOT_NUM; i++) {
		if (gx3201_dmx.slots[i] != NULL) {
			if ((gx3201_dmx.slots[i]->pid == pid) &&
				(gx3201_dmx.slots[i]->type == type) &&
				(gx3201_dmx.slots[i]->demux== demux) &&
				(pid != 0x1fff)) {
				gx3201_dmx.slots[i]->refcount++;
				return i;
			}
		}
	}
...
void gx3201_dmx_free_slot(struct dmx_slot *slot)
{
	int i;
	...
	if(DEMUX_SLOT_MUXTS == slot->type){
		...
		if(filter){
			gx3201_unlink_wts_sdc(demux,slot->id);
	...
	gx3201_clr_slot_cfg(slot);
	gx3201_disable_slot(slot);
	...
	if (slot->filter_num > 0) {
		for (i = 0; i < MAX_FILTER_NUM; i++) {
			if (NULL != slot->filters[i]) {
				gx3201_dmx_free_filter(slot->filters[i]);
				if (slot->filter_num == 0) {
					break;
				}
			}
...
int gx3201_dmx_alloc_filter(struct dmx_slot *slot)
{
	int i,ret;
	struct dmx_filter *filter = NULL;
	...
	gx3201_demux_delay_free(demux_device);
	...
	if(DEMUX_SLOT_MUXTS == slot->type){
		...
		gx3201_link_wts_sdc(demux,filter->phy_dfbuf_addr, filter->buffer_size, slot->id);
...
void gx3201_dmx_free_filter(struct dmx_filter *filter)
{
	...
	gx3201_clear_int_df(filter_index);
	gx3201_disable_filter(filter_index);
	gx3201_disable_pid_sel(slot->id, filter_index);
...
void gx3201_dmx_slot_config(struct dmx_slot *slot)
{
	struct dmx_demux *demux = slot->demux;

	if(demux->id == 2){
		gx3201_set_pid_slot(slot);
		return;
	}
	...
	if ((DEMUX_SLOT_PSI == slot->type) || (slot->type == DEMUX_SLOT_PES) || (slot->type == DEMUX_SLOT_PES_AUDIO)
		...
		gx3201_set_stop_unit(slot);
		gx3201_set_interrupt_unit(slot);
		gx3201_set_filter_type(slot);
		gx3201_set_crc_disable(slot);
		...
		if (DEMUX_SLOT_SPDIF == slot->type) {
			gx3201_set_audio_ac3();
			//gx3201_audioout_pcmtocompress();
		}
		gx3201_set_av_type(slot);
		gx3201_set_pts_bypass(slot);
		gx3201_set_avnum(slot);

	}
	gx3201_reset_cc(slot);
	gx3201_set_pts_to_sdram(slot);
	gx3201_set_type_sel(slot);
	gx3201_set_pid(slot);
	gx3201_set_av_out(slot);
	gx3201_set_ts_out(slot);
	gx3201_set_err_discard(slot);
	gx3201_set_dup_discard(slot);
}

void gx3201_dmx_filter_config(struct dmx_filter *filter)
{
	struct timeval tv;
	struct timezone tz;
	struct timeval *ftv;
	...
	gx3201_set_dfbuf(filter);
	gx3201_set_select(filter->slot, filter);
	gx3201_set_filtrate(filter);
	gx3201_set_filter_reset(filter);
}

void gx3201_dmx_enable_slot(struct dmx_slot *slot)
{
	gx3201_enable_slot(slot);
}

void gx3201_dmx_disable_slot(struct dmx_slot *slot)
{
	gx3201_disable_slot(slot);
}

void gx3201_dmx_enable_filter(struct dmx_filter *filter)
{
	struct timeval tv;
	struct timezone tz;
	struct timeval *ftv;
	...
	filter->pread_rd = 0;
	gx3201_clear_df_rw(filter->id);
	gx3201_enable_filter(filter);
}

void gx3201_dmx_disable_filter(struct dmx_filter *filter)
{
	struct timezone tz;
	gx_gettimeofday(&(filter->tv),&tz);
	filter->status = DMX_STATE_STOP;
	gx3201_disable_filter(filter->id);
}

int gx3201_dmx_alloc_ca(struct dmxdev *demux_device , int demux_id)
{
	int i;
	struct dmx_demux *demux = &demux_device->demuxs[demux_id];
...
void gx3201_dmx_ca_config(struct dmx_slot *slot, struct dmx_ca *ca)
{
	gx3201_set_descramble(ca, slot);
}

void gx3201_dmx_free_ca(struct dmx_ca *ca)
{
	int index = ca->id;
...
void gx3201_dmx_mtc_decrypt(gx_mtc_info *mtc_info_cfg,struct dmx_ca *ca)
{
	gx3201_mtc_decrypt(mtc_info_cfg,ca);
}

void gx3201_dmx_pcr_sync(struct dmxdev *demux_device, int demux_id, unsigned short pcr_pid)
{
	struct dmx_demux *demux = &demux_device->demuxs[demux_id];
	demux->pcr_pid = pcr_pid;

	gx3201_set_pcr_sync(demux);
}

static unsigned char tmp[4099];
unsigned int gx3201_dmx_filter_read(struct dmx_filter *filter, unsigned char *data_buf, unsigned int size)
{
	unsigned char crc = 0;
...
unsigned int gx3201_dmx_query_fifo(long long *state)
{
	unsigned int len,i;
	long long id = 0,st=0;
	struct dmx_filter *filter = NULL;
