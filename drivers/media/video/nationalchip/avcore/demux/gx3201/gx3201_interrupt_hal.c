...
static void gx3201_dmx_stc_isr(unsigned int int_dmx_ts)
{
	if(int_dmx_ts & (1<<24)){
		REG_SET_VAL(&(gx3201_demux_reg->int_dmx_ts), (1<<24));
		gx_printf("enter stc0 lock int\n");
	}
	if(int_dmx_ts & (1<<26)){
		REG_SET_VAL(&(gx3201_demux_reg->int_dmx_ts), (1<<26));
		gx_printf("enter stc0 loss int \n");
	}
	if(int_dmx_ts & (1<<25)){
		REG_SET_VAL(&(gx3201_demux_reg->int_dmx_ts), (1<<25));
		gx_printf("enter stc1 lock int\n");
	}
	if(int_dmx_ts & (1<<27)){
		REG_SET_VAL(&(gx3201_demux_reg->int_dmx_ts), (1<<27));
...
static void gx3201_dmx_df_dealwith(struct dmx_filter *filter)
{
	static int d = 0;
	unsigned int fifo_left=0;
	...
	df_pwrite = REG_GET_VAL(&(gx3201_demux_reg->df_waddr[(unsigned int)filter->id]));
	for(i=0;i<3;i++){
		df_pread = REG_GET_VAL(&(gx3201_demux_reg->df_addr[(unsigned int)filter->id].df_read_addr));
		df_pwrite_rd = REG_GET_VAL(&(gx3201_demux_reg->df_waddr[(unsigned int)filter->id]));
	...
	if(df_pwrite >= df_pread){
		...
		df_pread +=df_len;
		REG_SET_VAL(&(gx3201_demux_reg->df_addr[(unsigned int)filter->id].df_read_addr), df_pread);
	}
	else{
...
static void gx3201_dmx_df_al_isr(long long int_dmx_df)
{
	int i;
	...
	if (int_dmx_df & temp1) {
		if(df_isr_num <32)
			REG_SET_VAL(&(gx3201_demux_reg->int_df_buf_alfull_l), (1<<df_isr_num));
		else
			REG_SET_VAL(&(gx3201_demux_reg->int_df_buf_alfull_h), (1<<(df_isr_num-32)));
		filter = gx3201_dmx.filters[df_isr_num];
        if(filter == NULL || (filter->status == DMX_STATE_DELAY_FREE) || (filter->status == DMX_STATE_FREE))//free的接口已经释放,到中断就不处理已经释放的filter
            goto next;

		gx3201_dmx_df_dealwith(filter);
		goto set_event;
	}
#endif
next:
	for (i = 0; i < MAX_FILTER_NUM; i++) {
		...
		if (int_dmx_df & temp) {
			if(i <32)
				REG_SET_VAL(&(gx3201_demux_reg->int_df_buf_alfull_l), (1<<i));
			else
				REG_SET_VAL(&(gx3201_demux_reg->int_df_buf_alfull_h), (1<<(i-32)));
			filter = gx3201_dmx.filters[i];
			if(filter == NULL || (filter->status == DMX_STATE_DELAY_FREE) || (filter->status == DMX_STATE_FREE))//free的接口已经释放,到中断就不处理已经释放的filter
				continue;
			if(filter->slot == NULL)
				continue;
...
static void gx3201_dmx_df_isr(long long int_dmx_df)
{
	int i;
	long long temp = 0;//,temp1 = 0;
	//static int df_isr_num = 0;
	...
	if (int_dmx_df & temp1) {
		if(df_isr_num <32)
			REG_SET_VAL(&(gx3201_demux_reg->int_dmx_df_l), (1<<df_isr_num));
		else
			REG_SET_VAL(&(gx3201_demux_reg->int_dmx_df_h), (1<<(df_isr_num-32)));
		filter = gx3201_dmx.filters[df_isr_num];
	...
	for (i = 0; i < MAX_FILTER_NUM; i++) {
		...
		if (int_dmx_df & temp) {
			if(i <32)
				REG_SET_VAL(&(gx3201_demux_reg->int_dmx_df_l), (1<<i));
			else
				REG_SET_VAL(&(gx3201_demux_reg->int_dmx_df_h), (1<<(i-32)));
			filter = gx3201_dmx.filters[i];
			if(filter == NULL || (filter->status == DMX_STATE_DELAY_FREE) || (filter->status == DMX_STATE_FREE))//free的接口已经释放,到中断就不处理已经释放的filter
				continue;
			if(filter->slot == NULL)
...
void gx3201_dmx_avbuf_gate_isr(int int_avbuf)
{
	unsigned int i;
	unsigned int int_avbuf_en = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_av_buf_en));
	...
	for(i=0;i<8;i++) {
		if(int_avbuf & int_avbuf_en & (0x01<<i)){
			REG_SET_VAL(&(gx3201_demux_reg->int_dmx_av_buf),1<<i);

			avslot = gx3201_dmx.avslots[i];
			if(avslot != NULL)
			{
				avfifo = &gx3201_dmx.avfifo[i];
				gxav_sdc_rwaddr_get(avfifo->channel_id, &av_buf_rptr,&av_buf_wptr);
				REG_SET_VAL(&(gx3201_demux_reg->av_addr[i].av_read_addr), av_buf_rptr);

				av_buf_wptr = REG_GET_VAL(&(gx3201_demux_reg->av_addr[i].av_write_addr));
				gxav_sdc_Wptr_Set(avfifo->channel_id, av_buf_wptr);

				pts_buf_wptr = REG_GET_VAL(&(gx3201_demux_reg->pts_w_addr[i]));
				gx_dcache_inv_range(0, 0);
				gxav_sdc_Wptr_Set(avfifo->pts_channel_id, pts_buf_wptr);
				//	gx_printf("gate demux wptr 0x%x 0x%x \n",av_buf_wptr,pts_buf_wptr);
			}
		}
	}
}

void gx3201_dmx_avbuf_pes_isr(int int_avbuf)
{
	...
	for(i=0;i<8;i++) {
		if((int_avbuf>>24) & (int_avbuf_en>>24) & (0x01<<i)){
			REG_SET_VAL(&(gx3201_demux_reg->int_dmx_av_buf),1<<(i+24));
			avfifo = &gx3201_dmx.avfifo[i];
			gxav_sdc_rwaddr_get(avfifo->channel_id, &av_buf_rptr,&av_buf_wptr);
			REG_SET_VAL(&(gx3201_demux_reg->av_addr[i].av_read_addr), av_buf_rptr);
			av_buf_wptr = REG_GET_VAL(&(gx3201_demux_reg->av_addr[i].av_write_addr));
			av_buf_rptr = REG_GET_VAL(&(gx3201_demux_reg->av_addr[i].av_read_addr));
			size = REG_GET_VAL(&(gx3201_demux_reg->av_addr[i].av_buf_size));
			gxav_sdc_Wptr_Set(avfifo->channel_id, av_buf_wptr);
...
static void gx3201_dmx_tsw_dealwith(struct dmx_filter *filter)
{
	static int d = 0;
	unsigned int fifo_left=0;
	unsigned char *df_addr = NULL;
...
again:
	df_pwrite = REG_GET_VAL(&(gx3201_demux_reg->tsw_buf[tsw_id].tsw_write_addr));
	df_pread = REG_GET_VAL(&(gx3201_demux_reg->tsw_buf[tsw_id].tsw_read_addr));
	df_size = filter->buffer_size;
	df_addr = (unsigned char *)((unsigned int)filter->virtual_dfbuf_addr + df_pread);
	gx_dcache_inv_range(0,0);
	...
	if(df_pwrite >= df_pread){
		...
		if(df_len <= fifo_left){
			...
			if(d==0)
				gx_printf("d!!! :%d   :%d\n", df_len, fifo_left);
			d++;
			REG_SET_VAL(&(gx3201_demux_reg->tsw_buf[tsw_id].tsw_read_addr), df_pwrite/188*188);
			//gx3201_set_filter_reset(filter);
			return;
		}
		df_pread += df_len;
		REG_SET_VAL(&(gx3201_demux_reg->tsw_buf[tsw_id].tsw_read_addr), df_pread);
	}
	else{
	...
	df_pwrite = REG_GET_VAL(&(gx3201_demux_reg->tsw_buf[tsw_id].tsw_write_addr));
	df_pread = REG_GET_VAL(&(gx3201_demux_reg->tsw_buf[tsw_id].tsw_read_addr));
	gate = REG_GET_VAL(&(gx3201_demux_reg->tsw_buf[tsw_id].tsw_almost_full_gate));
	if(df_pwrite >= df_pread)
		df_len = df_pwrite - df_pread;
	else
		df_len = df_pwrite + df_size - df_pread;
...
static void gx3201_dmx_tsw_isr(long long int_tsw_buf)
{
	int i;
	long long temp;
	struct dmx_filter *filter = NULL;
	for (i = 0; i < MAX_FILTER_NUM; i++) {
		temp = (0x01ULL << i);
		if (int_tsw_buf & temp) {
			gx_spin_lock_irqsave(&(gx3201_dmx.demux_spin_lock),flag);
			thread_tsw_l &=~temp;
			gx_spin_unlock_irqrestore(&(gx3201_dmx.demux_spin_lock),flag);
			filter = gx3201_dmx.muxfilters[i];
			if(filter == NULL || (filter->status == DMX_STATE_DELAY_FREE) || (filter->status == DMX_STATE_FREE))//free的接口已经释放,到中断就不处理已经释放的filter
				continue;

			gx3201_dmx_tsw_dealwith(filter);
			goto set_event;
		}
	}
	if(i == MAX_FILTER_NUM)
...
static void gx3201_dmx_tsw_full_isr(long long int_tsw_buf_full)
{
	int i;
	long long temp;
	struct dmx_filter *filter = NULL;
...
static void gx3201_dmx_tsr_isr(unsigned int int_dmx_tsr)
{
	...
	if(int_dmx_tsr&0x01) {
		tsr_fifo = &gx3201_dmx.rfifo;
		REG_SET_VAL(&(gx3201_demux_reg->int_dmx_tsr_buf), 1);
		...
		tsr_rptr = REG_GET_VAL(&(gx3201_demux_reg->rts_sdc_rptr));
...
void gx3201_dmx_interrupt(filter_event_cb event_cb,void *priv)
{
	...
	demux_device = &gx3201_dmx;


	int_dmx_ts = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_ts));
	int_dmx_ts_en = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_ts_en));
	if (int_dmx_ts & int_dmx_ts_en) {
		DMX_DBG("int_dmx_ts = 0x%x\n", int_dmx_ts);
		gx3201_dmx_stc_isr(int_dmx_ts);
	}

	int_dmx_df_l = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_df_l));
	int_dmx_df_en_l = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_df_en_l));
	int_dmx_df_h = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_df_h));
	int_dmx_df_en_h = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_df_en_h));

	if ((int_dmx_df_l & int_dmx_df_en_l)||(int_dmx_df_h & int_dmx_df_en_h)) {
		DMX_DBG("int_dmx_df = 0x%x\n", int_dmx_df_l);
		gx3201_dmx_df_isr((((int_dmx_df_h & int_dmx_df_en_h)<<32)|(int_dmx_df_l & int_dmx_df_en_l)));
	}

	int_dmx_df_al_l = REG_GET_VAL(&(gx3201_demux_reg->int_df_buf_alfull_l));
	int_dmx_df_al_en_l = REG_GET_VAL(&(gx3201_demux_reg->int_df_buf_alfull_en_l));
	int_dmx_df_al_h = REG_GET_VAL(&(gx3201_demux_reg->int_df_buf_alfull_h));
	int_dmx_df_al_en_h = REG_GET_VAL(&(gx3201_demux_reg->int_df_buf_alfull_en_h));

	if ((int_dmx_df_al_l & int_dmx_df_al_en_l)||(int_dmx_df_al_h & int_dmx_df_al_en_h)) {
		DMX_DBG("int_dmx_df = 0x%x\n", int_dmx_df_al_l);
		gx3201_dmx_df_al_isr((((int_dmx_df_al_h & int_dmx_df_al_en_h)<<32)|(int_dmx_df_al_l & int_dmx_df_al_en_l)));
	}

	int_avbuf    = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_av_buf));
	int_avbuf_en = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_av_buf_en));
	if ((int_avbuf&0xff) & (int_avbuf_en&0xff)) {
		DMX_DBG("int_avbuf gate= 0x%x\n", int_avbuf);
		gx3201_dmx_avbuf_gate_isr(int_avbuf);
	}
	int_avbuf    = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_av_buf));
	int_avbuf_en = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_av_buf_en));
	if ((int_avbuf>>24) & (int_avbuf_en>>24)) {
		DMX_DBG("int_avbuf pes= 0x%x\n", int_avbuf);
		gx3201_dmx_avbuf_pes_isr(int_avbuf);
	}

	int_tsw_full_l = REG_GET_VAL(&(gx3201_demux_reg->int_tsw_buf_ful_l));
	int_tsw_full_l_en = REG_GET_VAL(&(gx3201_demux_reg->int_tsw_buf_ful_en_l));
	int_tsw_full_h = REG_GET_VAL(&(gx3201_demux_reg->int_tsw_buf_ful_h));
	int_tsw_full_h_en = REG_GET_VAL(&(gx3201_demux_reg->int_tsw_buf_ful_en_h));
	if((int_tsw_full_l & int_tsw_full_l_en)||(int_tsw_full_h & int_tsw_full_h_en)){
		...
		REG_SET_VAL(&(gx3201_demux_reg->int_tsw_buf_ful_l),int_tsw_full_l );
		REG_SET_VAL(&(gx3201_demux_reg->int_tsw_buf_ful_h),int_tsw_full_h );
		gx_spin_lock_irqsave(&(gx3201_dmx.demux_spin_lock),flag);
	...
	int_tsw_l_en = REG_GET_VAL(&(gx3201_demux_reg->int_tsw_buf_alful_en_l));
	int_tsw_h_en = REG_GET_VAL(&(gx3201_demux_reg->int_tsw_buf_alful_en_h));
	while(1){
		int_tsw_l = REG_GET_VAL(&(gx3201_demux_reg->int_tsw_buf_alful_l));
		int_tsw_h = REG_GET_VAL(&(gx3201_demux_reg->int_tsw_buf_alful_h));
		if((int_tsw_h & int_tsw_h_en)||(int_tsw_l & int_tsw_l_en)){
			DMX_DBG("int_tsw_l = 0x%llx\n", int_tsw_l);
			DMX_DBG("int_tsw_h = 0x%llx\n", int_tsw_l);
			REG_SET_VAL(&(gx3201_demux_reg->int_tsw_buf_alful_l),int_tsw_l);
			REG_SET_VAL(&(gx3201_demux_reg->int_tsw_buf_alful_h),int_tsw_h);
	...
	int_dmx_tsr = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_tsr_buf));
	int_dmx_tsr_en = REG_GET_VAL(&(gx3201_demux_reg->int_dmx_tsr_buf_en));
	if(int_dmx_tsr & int_dmx_tsr_en){
		DMX_DBG("int_tsr = 0x%x\n", int_dmx_tsr);
		gx3201_dmx_tsr_isr(int_dmx_tsr);
	}
}

void gx3201_demux_read_thread(void* p)
{
	...
	while(1){
		...
		if((thread_tsw_full_l & int_tsw_full_l_en)||(int_tsw_full_h & int_tsw_full_h_en)){
			DMX_PRINTF("int_tsw_full_l = 0x%llx\n", thread_tsw_full_l);
			DMX_DBG("int_tsw_full_h = 0x%llx\n", int_tsw_full_h);
			DMX_PRINTF("pw = %x\n",gx3201_demux_reg->tsw_buf[0].tsw_write_addr);
			DMX_PRINTF("pr = %x\n",gx3201_demux_reg->tsw_buf[0].tsw_read_addr);
			gx3201_dmx_tsw_full_isr((int_tsw_full_h<<32)|(thread_tsw_full_l));
		}
		if((int_tsw_h & int_tsw_h_en)||(thread_tsw_l & int_tsw_l_en)){
			DMX_DBG("int_tsw_l = 0x%llx\n", thread_tsw_l);
			DMX_DBG("int_tsw_h = 0x%llx\n", int_tsw_h);
			gx3201_dmx_tsw_isr((int_tsw_h<<32)|(thread_tsw_l));
		}
