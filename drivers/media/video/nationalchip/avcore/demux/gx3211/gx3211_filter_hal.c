...
void gx3211_set_filter_reset(struct dmx_filter *filter)
{
	...
	if (DEMUX_SLOT_MUXTS == filter->slot->type) {
		...
		if(filter->slot->demux->id == 0){
			if(i < 32)
				REG_CLR_BIT(&(reg->tsw_buf_en_0_l),i);
			else
				REG_CLR_BIT(&(reg->tsw_buf_en_0_h),i-32);


		}else if(filter->slot->demux->id == 1){
			if(i < 32)
				REG_CLR_BIT(&(reg->tsw_buf_en_1_l),i);
			else
				REG_CLR_BIT(&(reg->tsw_buf_en_1_h),i-32);
		...
		REG_SET_VAL(&(reg->tsw_buf[i].tsw_write_addr),0);
		REG_SET_VAL(&(reg->tsw_buf[i].tsw_read_addr),0);
		...
		if(filter->slot->demux->id == 0){
			if(i < 32)
				REG_SET_BIT(&(reg->tsw_buf_en_0_l),i);
			else
				REG_SET_BIT(&(reg->tsw_buf_en_0_h),i-32);

		}else if(filter->slot->demux->id == 1){
			if(i < 32)
				REG_SET_BIT(&(reg->tsw_buf_en_1_l),i);
			else
				REG_SET_BIT(&(reg->tsw_buf_en_1_h),i-32);
	...
	if(i < 32) {
		...
		temp = (0x01 << i);
		REG_SET_VAL(&(reg->int_df_crc_l), temp);
		REG_SET_VAL(&(reg->int_dmx_df_l), temp);
		REG_SET_VAL(&(reg->int_df_buf_alfull_l), temp);
	}
	else {
		i -= 32;
		...
		temp = (0x01 << i);
		REG_SET_VAL(&(reg->int_df_crc_h), temp);
		REG_SET_VAL(&(reg->int_dmx_df_h), temp);
		REG_SET_VAL(&(reg->int_df_buf_alfull_h), temp);
	}
...
void gx3211_clear_df_rw(struct reg_demux *reg,unsigned int index)
{
	...
	REG_SET_VAL(&(reg->df_waddr[index]),0);
}
...
void gx3211_clear_int_df(struct dmx_filter *filter)
{
	...
	int index = filter->id;
	if(index <32 ) {
		REG_CLR_BIT(&(reg->int_dmx_df_en_l), index);
		REG_SET_VAL(&(reg->int_dmx_df_l), (1<<index));
		REG_SET_VAL(&(reg->int_df_buf_alfull_l), (1<<index));
	}
	else {
		index -=32;
		REG_CLR_BIT(&(reg->int_dmx_df_en_h), index);
		REG_SET_VAL(&(reg->int_dmx_df_h), (1<<index));
		REG_SET_VAL(&(reg->int_df_buf_alfull_h), (1<<index));
	}
}
...
void gx3211_disable_filter(struct dmx_filter *filter)
{
	...
	int index = filter->id;
	if(index < 32 ) {
		REG_CLR_BIT(&(reg->df_on_l), index);
	}
	else {
		index -=32;
		REG_CLR_BIT(&(reg->df_on_h), index);
	}
...
void gx3211_disable_pid_sel(struct dmx_filter *filter,struct dmx_slot *slot)
{
	...
	if(filter_index <32 ) {
		REG_SET_BIT(&(reg->wen_mask), filter_index);
		REG_SET_VAL(&(reg->pid_cfg_sel[slot_index].pid_sel_l), 0);
		REG_CLR_BIT(&(reg->wen_mask), filter_index);
	}
	else {
		filter_index -=32;
		REG_SET_BIT(&(reg->wen_mask), filter_index);
		REG_SET_VAL(&(reg->pid_cfg_sel[slot_index].pid_sel_h), 0);
		REG_CLR_BIT(&(reg->wen_mask), filter_index);
	}
...
void gx3211_set_dfbuf(struct dmx_filter *filter)
{
	...
	REG_SET_VAL(&(reg->df_addr[index].df_start_addr), ((filter->phy_dfbuf_addr)));
	REG_SET_VAL(&(reg->df_addr[index].df_buf_size), filter->buffer_size);
	if(slot->type == DEMUX_SLOT_PES || slot->type == DEMUX_SLOT_PES_AUDIO
			|| slot->type == DEMUX_SLOT_PES_VIDEO || slot->type == DEMUX_SLOT_TS){
		REG_SET_VAL(&(reg->df_addr[index].df_almost_full_gate), ((filter->buffer_size)>>8));
		if(index < 32 )
			REG_SET_BIT(&(reg->int_df_buf_alfull_en_l), index);
		else
			REG_SET_BIT(&(reg->int_df_buf_alfull_en_h), (index-32));
	}
}
...
void gx3211_set_select(struct dmx_filter *filter,struct dmx_slot *slot)
{
	...
	if(filter_index < 32 ){
		REG_SET_BIT(&(reg->wen_mask), filter_index);
		REG_SET_VAL(&(reg->pid_cfg_sel[slot_index].pid_sel_l), (0x01 << filter_index));
		REG_CLR_BIT(&(reg->wen_mask), filter_index);
	}
	else{
		i = filter_index - 32;
		REG_SET_BIT(&(reg->wen_mask), i);
		REG_SET_VAL(&(reg->pid_cfg_sel[slot_index].pid_sel_h), (0x01 << i));
		REG_CLR_BIT(&(reg->wen_mask), i);
	}

	if (0 == slot->demux->id) {
		if(filter_index < 32 )
			REG_CLR_BIT(&(reg->df_sel_l), filter_index);
		else{
			i = filter_index - 32;
			REG_CLR_BIT(&(reg->df_sel_h), i);
		}
	} else if (1 == slot->demux->id) {
		if(filter_index <32 )
			REG_SET_BIT(&(reg->df_sel_l), filter_index);
		else{
			i = filter_index - 32;
			REG_SET_BIT(&(reg->df_sel_h), i);
		}
	}
}

void gx3211_set_match(struct dmx_filter *filter)
{
	struct dmx_slot*  slot = filter->slot;
	struct dmx_demux* demux = slot->demux;
	struct dmxdev*  dmxdev = demux->demux_device;
	...
	if (index < 32) {
		...
		for (j = 0; j <= filter_depth; j++) {
			if ((VERSION_BYTE_DEPTH == j) && (0 == eq_flag)) {
				...
				for (i = 0; i < NUM_MATCH_BIT; i++) {
					REG_SET_VAL(&(reg->m_byte_n[j].m_byte[i].m0_byte_l), 0x01 << index);
					REG_SET_VAL(&(reg->m_byte_n[j].m_byte[i].m1_byte_l), 0x01 << index);
				}
			}
		}
		...
		if (eq_flag) {
			REG_CLR_BIT(&(reg->df_nequal_on_l), index);
		} else {
			REG_SET_BIT(&(reg->df_nequal_on_l), index);
		}

		/*set filter depth */
		temp = REG_GET_VAL(&(reg->df_depth[index >> 3]));
		temp &= ~(0x0f << ((index & 0x07) << 2));
		temp |= (filter_depth << ((index & 0x07) << 2));
		REG_SET_VAL(&(reg->df_depth[index >> 3]), temp);
		...
		for (j = 0; j <= filter_depth; j++) {
			if ((VERSION_BYTE_DEPTH == j) && (0 == eq_flag)) {
				...
				for (i = 0; i < NUM_MATCH_BIT; i++) {
					REG_SET_VAL(&(reg->m_byte_n[j].m_byte[i].m0_byte_h), 0x01 << index);
					REG_SET_VAL(&(reg->m_byte_n[j].m_byte[i].m1_byte_h), 0x01 << index);
				}
			}
		}
		...
		/*set filtrate:eq or neq */
		if (eq_flag) {
			REG_CLR_BIT(&(reg->df_nequal_on_h), index);
		} else {
			REG_SET_BIT(&(reg->df_nequal_on_h), index);
		}

		/*set filter depth */
		temp = REG_GET_VAL(&(reg->df_depth[(filter->id) >> 3]));
		temp &= ~(0x0f << (((filter->id) & 0x07) << 2));
		temp |= (filter_depth << (((filter->id) & 0x07) << 2));
		REG_SET_VAL(&(reg->df_depth[(filter->id) >> 3]), temp);

		REG_CLR_BIT(&(reg->wen_mask), index);
	}
}

void gx3211_set_int_en(struct dmx_slot *slot, struct dmx_filter *filter)
{
	struct dmx_demux* demux = slot->demux;
	struct dmxdev*  dmxdev = demux->demux_device;
    struct reg_demux *reg = dmxdev->reg;
	...
	if(filter_index < 32) {
		REG_SET_BIT(&(reg->int_dmx_df_en_l), filter_index);
		REG_SET_BIT(&(reg->wen_mask), BIT_DEMUX_PID_CFG_INT_PER_UNIT);
		REG_SET_VAL(&(reg->pid_cfg_sel[slot_index].pid_cfg), DEMUX_PID_CFG_INT_PER_UNIT);
		REG_CLR_BIT(&(reg->wen_mask), BIT_DEMUX_PID_CFG_INT_PER_UNIT);
	}
	else {
		filter_index -= 32;
		REG_SET_BIT(&(reg->int_dmx_df_en_h), filter_index);
		REG_SET_BIT(&(reg->wen_mask), BIT_DEMUX_PID_CFG_INT_PER_UNIT);
		REG_SET_VAL(&(reg->pid_cfg_sel[slot_index].pid_cfg), DEMUX_PID_CFG_INT_PER_UNIT);
		REG_CLR_BIT(&(reg->wen_mask), BIT_DEMUX_PID_CFG_INT_PER_UNIT);
	}
...
void gx3211_set_filtrate(struct dmx_filter *filter)
{
	...
	gx3211_set_match(filter);
	gx3211_set_int_en(slot, filter);
}
...
void gx3211_enable_filter(struct dmx_filter *filter)
{
	...
	int index = filter->id;
	if(index < 32)
		REG_SET_BIT(&(reg->df_on_l), index);
	else {
		index -= 32;
		REG_SET_BIT(&(reg->df_on_h), index);
	}
}
...
int gx3211_filter_ts_status(struct dmx_filter *filter)
{
	...
	buffer_write_addr = REG_GET_VAL(&(reg->df_waddr[index]));
