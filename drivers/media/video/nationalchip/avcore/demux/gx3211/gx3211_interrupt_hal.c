...
static void gx3211_dmx_stc_isr(struct dmxdev *dmxdev, unsigned int int_dmx_ts)
{
	struct reg_demux *reg = dmxdev->reg;
	if(int_dmx_ts & (1<<24)){
		REG_SET_VAL(&(reg->int_dmx_ts), (1<<24));
		gx_printf("enter stc0 lock int\n");
	}
	if(int_dmx_ts & (1<<26)){
		REG_SET_VAL(&(reg->int_dmx_ts), (1<<26));
		gx_printf("enter stc0 loss int \n");
	}
	if(int_dmx_ts & (1<<25)){
		REG_SET_VAL(&(reg->int_dmx_ts), (1<<25));
		gx_printf("enter stc1 lock int\n");
	}
	if(int_dmx_ts & (1<<27)){
		REG_SET_VAL(&(reg->int_dmx_ts), (1<<27));
...
static void gx3211_dmx_df_dealwith(struct reg_demux *reg,struct dmx_filter *filter)
{
	static int d = 0;
	unsigned int fifo_left=0;
	...
	df_pwrite = REG_GET_VAL(&(reg->df_waddr[(unsigned int)filter->id]));
	for(i=0;i<3;i++){
		df_pread = REG_GET_VAL(&(reg->df_addr[(unsigned int)filter->id].df_read_addr));
		df_pwrite_rd = REG_GET_VAL(&(reg->df_waddr[(unsigned int)filter->id]));
	...
	if(df_pwrite >= df_pread){
		...
		df_pread +=df_len;
		REG_SET_VAL(&(reg->df_addr[(unsigned int)filter->id].df_read_addr), df_pread);
	}
	else{
...
static void gx3211_dmx_df_al_isr(struct dmxdev *dmxdev,long long int_dmx_df)
{
	struct reg_demux *reg = dmxdev->reg;
	int i;
	...
	if (int_dmx_df & temp1) {
		...
		filter = dmxdev->filters[df_isr_num];
		if((filter == NULL) || (filter->status == DMX_STATE_DELAY_FREE) || (filter->status == DMX_STATE_FREE))
			goto next;

		gx3211_dmx_df_dealwith(reg,filter);
		goto set_event;
	}
#endif
next:
...
static void gx3211_dmx_df_isr(struct dmxdev *dmxdev,long long int_dmx_df)
{
	struct reg_demux *reg = dmxdev->reg;
	int i;
	long long temp = 0;
...
void gx3211_dmx_avbuf_gate_isr(struct dmxdev *dmxdev,int int_avbuf)
{
	struct reg_demux *reg = dmxdev->reg;
	unsigned int i;
	unsigned int int_avbuf_en = REG_GET_VAL(&(reg->int_dmx_av_buf_en));
	...
	for(i=0;i<MAX_AVBUF_NUM;i++) {
		if(int_avbuf & int_avbuf_en & (0x01<<i)){
			...
			avslot = dmxdev->avslots[i];
			avfifo = &(dmxdev->avfifo[i]);
			gxav_sdc_rwaddr_get(avfifo->channel_id, &av_buf_rptr,&av_buf_wptr);
			REG_SET_VAL(&(reg->av_addr[i].av_read_addr), av_buf_rptr);

			av_buf_wptr = REG_GET_VAL(&(reg->av_addr[i].av_write_addr));
			gxav_sdc_Wptr_Set(avfifo->channel_id, av_buf_wptr);

			pts_buf_wptr = REG_GET_VAL(&(reg->pts_w_addr[i]));
			gx_dcache_inv_range(0, 0);
...
void gx3211_dmx_avbuf_pes_isr(struct dmxdev *dmxdev,int int_avbuf)
{
	struct reg_demux *reg = dmxdev->reg;
	...
	for(i=0;i<8;i++) {
		if((int_avbuf>>24) & (int_avbuf_en>>24) & (0x01<<i)){
			...
			REG_SET_VAL(&(reg->av_addr[i].av_read_addr), av_buf_rptr);
			av_buf_wptr = REG_GET_VAL(&(reg->av_addr[i].av_write_addr));
			av_buf_rptr = REG_GET_VAL(&(reg->av_addr[i].av_read_addr));
			size = REG_GET_VAL(&(reg->av_addr[i].av_buf_size));
			gxav_sdc_Wptr_Set(avfifo->channel_id, av_buf_wptr);
...
static void gx3211_dmx_tsw_dealwith(struct reg_demux *reg, struct dmx_filter *filter)
{
	static int d = 0;
	unsigned int fifo_left=0;
	unsigned char *df_addr = NULL;
...
again:
	df_pwrite = REG_GET_VAL(&(reg->tsw_buf[tsw_id].tsw_write_addr));
	df_pread = REG_GET_VAL(&(reg->tsw_buf[tsw_id].tsw_read_addr));
	df_size = filter->buffer_size;
	df_addr = (unsigned char *)((unsigned int)filter->virtual_dfbuf_addr + df_pread);
	gx_dcache_inv_range(0,0);
	...
	if(df_pwrite >= df_pread){
		df_len = (df_pwrite - df_pread)/align*align;
		if(df_len <= fifo_left){
			if (df_len == 0)
				return;
			gxfifo_put(fifo, df_addr, df_len);
			...
			if(d==0)
				gx_printf("d!!! :%d   :%d\n", df_len, fifo_left);
			d++;
			REG_SET_VAL(&(reg->tsw_buf[tsw_id].tsw_read_addr), df_pwrite/align*align);
			//gx3211_set_filter_reset(filter);
			return;
		}
		df_pread += df_len;
		REG_SET_VAL(&(reg->tsw_buf[tsw_id].tsw_read_addr), df_pread);
	}
	else{
		df_len = (df_pwrite + df_size - df_pread)/align*align;
		if(df_len <= fifo_left){
	...
	df_pwrite = REG_GET_VAL(&(reg->tsw_buf[tsw_id].tsw_write_addr));
	df_pread = REG_GET_VAL(&(reg->tsw_buf[tsw_id].tsw_read_addr));
	gate = REG_GET_VAL(&(reg->tsw_buf[tsw_id].tsw_almost_full_gate));
	if(df_pwrite >= df_pread)
		df_len = df_pwrite - df_pread;
	else
		df_len = df_pwrite + df_size - df_pread;
...
static void gx3211_dmx_tsw_isr(struct dmxdev *dmxdev,long long int_tsw_buf)
{
	struct reg_demux *reg = dmxdev->reg;
	int i;
	long long temp;
	...
	for (i = 0; i < MAX_FILTER_NUM; i++) {
		temp = (0x01ULL << i);
		if (int_tsw_buf & temp) {
			gx_spin_lock_irqsave(&(dmxdev->demux_spin_lock),flag);
			dmxdev->thread_tsw &=~temp;
			gx_spin_unlock_irqrestore(&(dmxdev->demux_spin_lock),flag);
			filter = dmxdev->muxfilters[i];
			if(filter == NULL || (filter->status == DMX_STATE_DELAY_FREE) || (filter->status == DMX_STATE_FREE))
				continue;

			gx3211_dmx_tsw_dealwith(reg,filter);
			goto set_event;
		}
	}
	if(i == MAX_FILTER_NUM)
...
static void gx3211_dmx_tsw_full_isr(struct dmxdev *dmxdev,long long int_tsw_buf_full)
{
	int i;
	long long temp;
	struct dmx_filter *filter = NULL;
...
static void gx3211_dmx_tsr_isr(struct dmxdev *dmxdev,unsigned int int_dmx_tsr)
{
	struct reg_demux *reg = dmxdev->reg;
	...
	if(int_dmx_tsr&0x01) {
		...
		tsr_rptr = REG_GET_VAL(&(reg->rts_sdc_rptr));
...
void gx3211_dmx_interrupt(filter_event_cb event_cb,void *priv)
{
	...
	long long int_tsw_l,int_tsw_l_en;
	long long int_tsw_h,int_tsw_h_en;
	long long int_tsw_full_l,int_tsw_full_l_en;
	long long int_tsw_full_h,int_tsw_full_h_en;

	unsigned int int_dmx_tsr,int_dmx_tsr_en;

	struct dmxdev *dmxdev =(struct dmxdev *)priv;
	struct reg_demux *reg = dmxdev->reg;

	int_dmx_ts = REG_GET_VAL(&(reg->int_dmx_ts));
	int_dmx_ts_en = REG_GET_VAL(&(reg->int_dmx_ts_en));
	if (int_dmx_ts & int_dmx_ts_en) {
		DMX_DBG("int_dmx_ts = 0x%x\n", int_dmx_ts);
		gx3211_dmx_stc_isr(dmxdev,int_dmx_ts);
	}
	...
	if ((int_dmx_df_l & int_dmx_df_en_l)||(int_dmx_df_h & int_dmx_df_en_h)) {
		...
		gx3211_dmx_df_isr(dmxdev,(((int_dmx_df_h & int_dmx_df_en_h)<<32)|((int_dmx_df_l & int_dmx_df_en_l)&0xffffffff)));
	}
	...
	if ((int_dmx_df_al_l & int_dmx_df_al_en_l)||(int_dmx_df_al_h & int_dmx_df_al_en_h)) {
		DMX_DBG("int_dmx_df = 0x%x\n", int_dmx_df_al_l);
		gx3211_dmx_df_al_isr(dmxdev,(((int_dmx_df_al_h & int_dmx_df_al_en_h)<<32)|(int_dmx_df_al_l & int_dmx_df_al_en_l)));
	}
	...
	if ((int_avbuf&0xff) & (int_avbuf_en&0xff)) {
		...
		gx3211_dmx_avbuf_gate_isr(dmxdev,int_avbuf);
	}
	int_avbuf    = REG_GET_VAL(&(reg->int_dmx_av_buf));
	int_avbuf_en = REG_GET_VAL(&(reg->int_dmx_av_buf_en));
	if ((int_avbuf>>24) & (int_avbuf_en>>24)) {
		DMX_DBG("int_avbuf pes= 0x%x\n", int_avbuf);
		gx3211_dmx_avbuf_pes_isr(dmxdev,int_avbuf);
	}

	int_tsw_full_l = REG_GET_VAL(&(reg->int_tsw_buf_ful_l));
	int_tsw_full_l_en = REG_GET_VAL(&(reg->int_tsw_buf_ful_en_l));
	int_tsw_full_h = REG_GET_VAL(&(reg->int_tsw_buf_ful_h));
	int_tsw_full_h_en = REG_GET_VAL(&(reg->int_tsw_buf_ful_en_h));
	if((int_tsw_full_l & int_tsw_full_l_en)||(int_tsw_full_h & int_tsw_full_h_en)){
		...
		REG_SET_VAL(&(reg->int_tsw_buf_ful_l),int_tsw_full_l );
		REG_SET_VAL(&(reg->int_tsw_buf_ful_h),int_tsw_full_h );
		gx_spin_lock_irqsave(&(dmxdev->demux_spin_lock),flag);
		dmxdev->thread_tsw_full |=  (int_tsw_full_h<<32)|int_tsw_full_l;
		gx_spin_unlock_irqrestore(&(dmxdev->demux_spin_lock),flag);
		gx_sem_post(&(dmxdev->sem_id));
	}

	int_tsw_l_en = REG_GET_VAL(&(reg->int_tsw_buf_alful_en_l));
	int_tsw_h_en = REG_GET_VAL(&(reg->int_tsw_buf_alful_en_h));
	while(1){
		int_tsw_l = REG_GET_VAL(&(reg->int_tsw_buf_alful_l));
		int_tsw_h = REG_GET_VAL(&(reg->int_tsw_buf_alful_h));
		if((int_tsw_h & int_tsw_h_en)||(int_tsw_l & int_tsw_l_en)){
			...
			gx_spin_lock_irqsave(&(dmxdev->demux_spin_lock),flag);
			dmxdev->thread_tsw |= (int_tsw_h<<32)|int_tsw_l;
			gx_spin_unlock_irqrestore(&(dmxdev->demux_spin_lock),flag);
			gx_sem_post(&(dmxdev->sem_id));
		}
		else
	...
	if(int_dmx_tsr & int_dmx_tsr_en){
		...
		gx3211_dmx_tsr_isr(dmxdev,int_dmx_tsr);
	}
}

void gx3211_demux_read_thread(void* p)
{
	struct dmxdev *dmxdev = (struct dmxdev *)p;
	struct reg_demux *reg = dmxdev->reg;
	while(1){
		DMX_DBG("===========wait \n");
		gx_sem_wait_timeout(&(dmxdev->sem_id),6000);
		if(dmxdev->thread_tsw_full){
			DMX_PRINTF("int_tsw_full_l = 0x%llx\n", dmxdev->thread_tsw_full);
			DMX_DBG("int_tsw_full_h = 0x%llx\n", int_tsw_full_h);
			DMX_PRINTF("pw = %x\n",reg->tsw_buf[0].tsw_write_addr);
			DMX_PRINTF("pr = %x\n",reg->tsw_buf[0].tsw_read_addr);
			gx3211_dmx_tsw_full_isr(dmxdev,dmxdev->thread_tsw_full);
		}
		if(dmxdev->thread_tsw){
			DMX_DBG("int_tsw_l = 0x%llx\n", dmxdev->thread_tsw);
			DMX_DBG("int_tsw_h = 0x%llx\n", int_tsw_h);
			gx3211_dmx_tsw_isr(dmxdev,dmxdev->thread_tsw);
		}
