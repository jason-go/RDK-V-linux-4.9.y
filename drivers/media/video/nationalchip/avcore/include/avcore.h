...
#define GXAV_MAX_MODULE          ( GXAV_MOD_MAX )
...
struct gxav_device_resource {
	struct gxav_module_resource module[GXAV_MOD_MAX];
};
...
struct gxav_module_ops {
	...
	const char*           irq_names[GXAV_MAX_IRQ];
	unsigned int          count;
	unsigned int          event_mask;
	void*                 priv;

	int (*setup)          (struct gxav_device *dev, struct gxav_module_resource *res);
	...
	int (*read)           (struct gxav_module *module, void *buf, unsigned int size);
	int (*write)          (struct gxav_module *module, void *buf, unsigned int size);
	...
	struct gxav_module_inode *(*interrupts[GXAV_MAX_IRQ])(struct gxav_module_inode *inode, int irq);
};
...
enum gxav_module_event {
	GXAV_MODULE_EVENT_EMPTY,
	GXAV_MODULE_EVENT_FULL,
};
