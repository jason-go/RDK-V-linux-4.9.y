...
extern struct pp_ops gx3201pp_ops;
struct gxav_module_ops gx3201_videodec_module;

static int video_hal_setup(struct gxav_device *dev, struct gxav_module_resource *res)
{
	gx3201_videodec_module.irqs[0] = VIDEO_BODA_INIT_SRC = res->irqs[0];
	gx3201_videodec_module.irqs[1] = VIDEO_VPU_INC_SRC   = res->irqs[1];
	gx3201_videodec_module.irqs[2] = VIDEO_PP_INC_SRC    = res->irqs[2];
	gx3201_videodec_module.interrupts[res->irqs[0]] = video_hal_interrupt;
	gx3201_videodec_module.interrupts[res->irqs[1]] = video_hal_interrupt;
	gx3201_videodec_module.interrupts[res->irqs[2]] = video_hal_interrupt;
