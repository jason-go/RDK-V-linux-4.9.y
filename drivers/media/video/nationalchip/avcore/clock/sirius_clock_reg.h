#ifndef __SIRIUS_CLOCK_REG_H__
#define __SIRIUS_CLOCK_REG_H__

#include "gxav_bitops.h"

#define CLOCK_BASE_ADDR (0x89400000)
...
#define DTO_PLL_F	(REG_GET_FIELD(&(sirius_clock_reg->cfg_pll4), DTO_PLL_F_MASK, BIT_DTO_PLL_F))
#define DTO_PLL_R	(REG_GET_FIELD(&(sirius_clock_reg->cfg_pll4), DTO_PLL_R_MASK, BIT_DTO_PLL_R))
#define DTO_PLL_OD	(REG_GET_FIELD(&(sirius_clock_reg->cfg_pll4), DTO_PLL_OD_MASK, BIT_DTO_PLL_OD))

struct clock_regs {
	unsigned int   cfg_mpeg_cold_reset_norm;
	unsigned int   cfg_mpeg_cold_reset_set;
	unsigned int   cfg_mpeg_cold_reset_clr;
	unsigned int   cfg_mpeg_hot_reset_norm;           /* 0x0c */
	unsigned int   cfg_mpeg_hot_reset_set;
	unsigned int   cfg_mpeg_hot_reset_clr;
	unsigned int   cfg_mepg_clk_inhibit_norm;
...
#define CFG_AUDIO_OR_SET_DTO(base,val)                   \
	do {                                                    \
		...
		REG_SET_VAL(&(base->cfg_dto7 ), ( vCFG_DTO_WSEL | ((val) & 0x3fffffff))); \
		REG_SET_VAL(&(base->cfg_dto7 ), ( vCFG_DTO_WSEL | vCFG_DTO_LOAD | ((val) & 0x3fffffff))); \
		REG_SET_VAL(&(base->cfg_dto7 ), ( vCFG_DTO_WSEL | ((val) & 0x3fffffff))); \
	} while(0)
...
#define CFG_AUDIO_LODAC_CLOCK_ENABLE(base) \
	REG_SET_VAL(&(base->cfg_mpeg_clk_inhibit2_clr), 1<<7)

#define CFG_AUDIO_LODAC_CLOCK_DISABLE(base) \
	REG_SET_VAL(&(base->cfg_mpeg_clk_inhibit2_set), 1<<7)
...
#define CFG_VIDEOOUT1_HOT_SET(base) \
	REG_SET_VAL(&(base->cfg_mpeg_hot_rst_2_1set),1<<4)

#define CFG_VIDEOOUT1_HOT_CLR(base) \
	REG_SET_VAL(&(base->cfg_mpeg_hot_rst_2_1clr), 1<<4)

#define CFG_SVPU_HOT_SET(base) \
	REG_SET_VAL(&(base->cfg_mpeg_hot_rst_2_1set), 1<<8)

#define CFG_JPEG_HOT_SET(base) \
	REG_SET_VAL(&(base->cfg_mpeg_hot_reset_set), 1<<2)

#define CFG_SVPU_HOT_CLR(base) \
	REG_SET_VAL(&(base->cfg_mpeg_hot_rst_2_1clr), 1<<8)

#define CFG_JPEG_HOT_CLR(base) \
	REG_SET_VAL(&(base->cfg_mpeg_hot_reset_clr), 1<<2)

#define CFG_VPU_HOT_SET(base)							\
	REG_SET_VAL(&(base->cfg_mpeg_hot_reset_set), 1<<9)

#define CFG_VPU_HOT_CLR(base)							\
	REG_SET_VAL(&(base->cfg_mpeg_hot_reset_clr), 1<<9)

#define CFG_VIDEO1_COLD_SET(base) \
	REG_SET_VAL(&(sirius_clock_reg->cfg_mpeg_cold_rst_2_1set), 1<<4)

#define CFG_VIDEO1_COLD_CLR(base) \
	REG_SET_VAL(&(sirius_clock_reg->cfg_mpeg_cold_rst_2_1clr), 1<<4)
...
#define CFG_FRONTEND_DEMOD_CLK(base, div, source_sel) \
	do { \
		REG_CLR_BITS(&(base->cfg_clock2), (0x01<<12)); \
		REG_SET_BITS(&(base->cfg_clock2), (0x01<<12)); \
		REG_CLR_BITS(&(base->cfg_clock2), (0x3F<<4)); \
		REG_SET_BITS(&(base->cfg_clock2), (div<<4)); \
		REG_CLR_BITS(&(base->cfg_clock2), (0x01<<11)); \
		REG_SET_BITS(&(base->cfg_clock2), (0x01<<11)); \
		REG_CLR_BITS(&(base->cfg_source_sel), (0x01<<30)); \
		REG_SET_BITS(&(base->cfg_source_sel), (source_sel<<30)); \
	} while(0)

#define CFG_FRONTEND_FEC_CLK(base, div, source_sel) \
	do { \
		REG_CLR_BITS(&(base->cfg_clock2), (0x01<<21)); \
		REG_SET_BITS(&(base->cfg_clock2), (0x01<<21)); \
		REG_CLR_BITS(&(base->cfg_clock2), (0x3F<<13)); \
		REG_SET_BITS(&(base->cfg_clock2), (div<<13)); \
		REG_CLR_BITS(&(base->cfg_clock2), (0x01<<20)); \
		REG_SET_BITS(&(base->cfg_clock2), (0x01<<20)); \
		REG_CLR_BITS(&(base->cfg_source_sel), (0x01<<29)); \
		REG_SET_BITS(&(base->cfg_source_sel), (source_sel<<29)); \
	} while(0)

#define CFG_FRONTEND_ADC_CLK(base, div, source_sel) \
	do { \
		REG_CLR_BITS(&(base->cfg_clock2), (0x01<<30)); \
		REG_SET_BITS(&(base->cfg_clock2), (0x01<<30)); \
		REG_CLR_BITS(&(base->cfg_clock2), (0x3F<<22)); \
		REG_SET_BITS(&(base->cfg_clock2), (div<<22)); \
		REG_CLR_BITS(&(base->cfg_clock2), (0x01<<29)); \
		REG_SET_BITS(&(base->cfg_clock2), (0x01<<29)); \
		REG_CLR_BITS(&(base->cfg_source_sel), (0x01<<27)); \
		REG_SET_BITS(&(base->cfg_source_sel), (source_sel<<27)); \
	} while(0)

#define CFG_FRONTEND_ADC_INIT(base) \
	do { \
		unsigned int i = 1000; \
		REG_CLR_BITS(&(base->cfg_adc_config0), (0x03<<18)); \
		REG_CLR_BITS(&(base->cfg_adc_config0), (0x01<<29)); \
		while(i--); \
		REG_SET_BITS(&(base->cfg_adc_config0), (0x01<<29)); \
		REG_SET_BITS(&(base->cfg_adc_config0), (0x03<<18)); \
	} while(0)
