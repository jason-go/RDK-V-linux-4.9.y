...
const u16 I2C_MIN_FS_SCL_HIGH_TIME =852;//600;
const u16 I2C_MIN_FS_SCL_LOW_TIME = 1874;//1300;
const u16 I2C_MIN_SS_SCL_HIGH_TIME = 6394;//4500; /*4000;*/
const u16 I2C_MIN_SS_SCL_LOW_TIME = 7389;//5200; /*4700;*/
const u32 I2C_DIV_FACTOR = 100000;

u8 edid_mBlocksNo;

u8 edid_mTotalBlocksNr;
/**
...
u8 edid_mBufferFourBlocks[0x200];
/**
...
hdrStaticMetadataDataBlock_t edid_mHdrMetadataDataBlock;
...
int edid_Initialize(u16 baseAddr, u16 sfrClock)
{
	...
	edid_Reset(baseAddr);
	/* set clock division */
	halEdid_MasterClockDivision(baseAddr + I2CM_BASE_ADDR, 0x01);//slow mode, 0x09 fast mode
...
int edid_ReadRequestPatch(u16 baseAddr)
{
...
u8 edid_EventHandler(u16 baseAddr, int hpd, u8 state)
{
		...
		else
		{
			...
			if (edid_mCurrAddress >= sizeof(edid_mBuffer))
			{
				...
				if (edid_mBlockSum % 0x100 == 0)
				{
					...
					for( bCounter=0; bCounter< 128; bCounter++)
					{
						edid_mBufferFourBlocks[(u16)(bCounter+edid_mCurrBlockNo*128)] = edid_mBuffer[bCounter];
						//log_notice("block parsing");
					}
					edid_mCurrAddress = 0;
					edid_mCurrBlockNo++;
					edid_mBlockSum = 0;
	...
	if (edid_mStatus == EDID_DONE)
	{
		edid_mTotalBlocksNr = edid_mBlocksNo;
		edid_mBlocksNo = 1;
		edid_mCurrBlockNo = 0;
		edid_mCurrAddress = 0;
		edid_mBlockSum = 0;
...
edid_status_t edid_GetStatus(void)
{
	LOG_TRACE();
	return edid_mStatus;
}
...
int edid_ParseDataBlock(u16 baseAddr, u8 * data)
{
	...
	case 0x7:
	{
		...
		case 0x06: /* HDR Static Metadata Data Block */
			if (hdrStaticMetadataDataBlock_Parse(&edid_mHdrMetadataDataBlock, data)
					!= TRUE)
			{
				LOG_WARNING("HDR Data Block corrupt");
			}
			break;
...
u16 edid_GetVicSupported3dStructs(u16 baseAddr, hdmivsdb_t *vsdb, u8 vic)
{
	int i;
	for (i = 0; i < edid_mSvdIndex; i++)
	{	/* get the vics index */
		if (edid_mSvd[i].mCode == vic)
		{
			return hdmivsdb_GetIndexSupported3dStructs(vsdb, i);
		}
	}
	return 0;
}

int edid_Get3dStructVics(u16 baseAddr, hdmivsdb_t *vsdb, u8 struct3d, u8 * vics, unsigned vicsLength)
{
	unsigned i;
	u16 indexes = hdmivsdb_Get3dStructIndexes(vsdb, struct3d);
	unsigned vicCounter = 0;
	unsigned maxCount = (vicsLength > edid_mSvdIndex)? edid_mSvdIndex: vicsLength;
	for (i = 0; i < maxCount; i++)
	{
		if ((indexes & (1 << i)) > 0)
		{
			vics[vicCounter] = edid_mSvd[i].mCode;
			vicCounter++;
		}
	}
	return i;
...
int edid_VicSupports3dStruct(u16 baseAddr, hdmivsdb_t *vsdb, u8 vic, u8 struct3d)
{
	int i;
	for (i = 0; i < edid_mSvdIndex; i++)
	{	/* get the vics index */
		if (edid_mSvd[i].mCode == vic)
		{
			return hdmivsdb_IndexSupports3dStruct(vsdb, i, struct3d);
		}
	}
	return 0;
}

u8 edid_Get3dStructDetail(u16 baseAddr, hdmivsdb_t *vsdb, u8 vic, u8 struct3d)
{
	int i;
	for (i = 0; i < edid_mSvdIndex; i++)
	{	/* get the vics index */
		if (edid_mSvd[i].mCode == vic)
		{
			return hdmivsdb_Get3dStructDetail(vsdb, i, struct3d);
		}
	}
	return 0;
}

int edid_3dStructHasDetail(u16 baseAddr, hdmivsdb_t *vsdb, u8 vic, u8 struct3d)
{
	int i;
	for (i = 0; i < edid_mSvdIndex; i++)
	{	/* get the vics index */
		if (edid_mSvd[i].mCode == vic)
		{
			return hdmivsdb_3dStructHasDetail(vsdb, i, struct3d);
		}
	}
	return FALSE;
}
...
{
	int i;
	*nrBlocks = edid_mTotalBlocksNr;
	for(i = 0; i < edid_mTotalBlocksNr*128; i++)
	{
		rawEdid[i] = edid_mBufferFourBlocks[i];
	}
}

int edid_IsHdmi(void)
{
	return edid_mHdmivsdb.mId == 0x000C03;
}

int edid_GetPhysicalAddr(void)
{
	return edid_mHdmivsdb.mPhysicalAddress;
}

int edid_SupportsHDR10(u16 baseAddr)
{
	return edid_mHdrMetadataDataBlock.mValid ? hdrStaticMetadataDataBlock_SupportsHDR10(&edid_mHdrMetadataDataBlock) : 0;
}

int edid_SupportsHLG(u16 baseAddr)
{
	return edid_mHdrMetadataDataBlock.mValid ? hdrStaticMetadataDataBlock_SupportsHLG(&edid_mHdrMetadataDataBlock) : 0;
}
