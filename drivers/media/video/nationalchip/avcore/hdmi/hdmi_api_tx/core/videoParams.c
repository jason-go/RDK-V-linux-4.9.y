...
void videoParams_UpdateCscCoefficientsCorrect(videoParams_t *params)
{
	u16 i = 0;
...
{
	if (params->mEncodingOut == RGB) {
		videoParams_UpdateCscCoefficientsCorrect(params);
	}
	else {
...
{
	u16 i = 0;
	if (!videoParams_IsColorSpaceConversion(params) || params->mEncodingOut == YCC422)
	{
		for (i = 0; i < 4; i++)
		{
			params->mCscA[i] = 0;
