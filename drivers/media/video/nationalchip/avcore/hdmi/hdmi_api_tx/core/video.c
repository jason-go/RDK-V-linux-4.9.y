...
int video_FrameComposer(u16 baseAddr, videoParams_t *params, u8 dataEnablePolarity, u8 hdcp)
{
	...
	if (videoParams_GetHdmiVideoFormat(params) == 2)
	{
		...
		{	/* 3d data frame packing is transmitted as a progressive format */
			halFrameComposerVideo_VBlankOsc(baseAddr + FC_BASE_ADDR, 0);
			halFrameComposerVideo_Interlaced(baseAddr + FC_BASE_ADDR, 0);
			if (dtd_GetInterlaced(dtd))
			{
				halFrameComposerVideo_VActive(baseAddr + FC_BASE_ADDR,
						(dtd_GetVActive(dtd) << 2) + 3 * dtd_GetVBlanking(dtd) + 2);
			}
			else
			{
				halFrameComposerVideo_VActive(baseAddr + FC_BASE_ADDR,
						(dtd_GetVActive(dtd) << 1) + dtd_GetVBlanking(dtd));
			}
		}
		else
		{
...
void video_DviOrHdmi(u16 baseAddr, u8 hdmi_flag)
{
	halFrameComposerVideo_DviOrHdmi(baseAddr + FC_BASE_ADDR, hdmi_flag);
}
