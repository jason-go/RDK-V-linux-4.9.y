...
typedef enum
{
	HDMI_TX_EDID_READ_EVENT = 0, /** calls registered callback when through reading
									E-EDID information */
	HDMI_TX_HPD_EVENT, /** calls registered callback for any rising edge
									or falling edge events */
	HDMI_TX_DMA_DONE_EVENT,	/** calls registered callback when a DMA is done reading */
	HDMI_TX_DMA_ERROR_EVENT,	/** calls registered callback when a DMA error occurs */
	HDMI_TX_KSV_LIST_EVENT, /** calls registered callback when ksv list is updated
								with buffer_t struct, having:
								- KSV list (5 bytes each entry) first
								- Bstatus and M0 (10 bytes)
								- V' 20 bytes */
	HDMI_TX_HDCP_AUTH_EVENT,
	HDMI_TX_MAX_EVENTS		/* out of bound */
}
event_t;
/**
 ...
 * @param param dummy argumenHDMI_TX_MAX_EVENTSnter to void
 */
 ...
 * @param idEvent the assigned ID of type EventType (listed in api.h)
 * @param handler of the function to be called when a certain event is
 * raised. (handlers receive a void * param, which the HPD state will be passed
 * as an argument)
 * @return TRUE when successful
 * @note This function must be called after API initialization
 */
//int api_EventEnable(event_t idEvent, handler_t handler);
...
/**
* @return TRUE if HDR10 is supported by sink
*/
int api_EdidSupportsHDR10(void);
/**
* @return TRUE if HLG is supported by sink
*/
int api_EdidSupportsHLG(void);


/** Get all 3D structures supported for a certain video mode by the sink
 * @param vsdb HDMI VSDB structure
 * @param vic video mode CEA 861 code
 * @return 16-bit integer where each bit location indicates a 3D structure.
 * a set bit indicates the support of the structure at that location.
 */

u16 api_GetVicSupported3dStructs(hdmivsdb_t *vsdb, u8 vic);
/** Get all the video modes supported for a certain 3D mode
 * @param vsdb HDMI VSDB structure
 * @param struct3d code for the 3D structure
 * @param vics buffer to store all the CEA 861 video codes
 * @param vicsLength the maximum number of codes the buffer can hold
 * @return the number of video codes stored in the vics buffer
 */
int api_Get3dStructVics(hdmivsdb_t *vsdb, u8 struct3d, u8 * vics, unsigned vicsLength);
/** Check if a certain video mode is supported for a certain 3D structure
 * @param vsdb HDMI VSDB structure
 * @param vic video mode CEA 861 code
 * @param struct3d code for the 3D structure
 * @returns TRUE if 3D strucutre is supported
 */
int api_VicSupports3dStruct(hdmivsdb_t *vsdb, u8 vic, u8 struct3d);
/** Get detail of a 3D format, if any
 * @param vsdb HDMI VSDB structure
 * @param vic video mode CEA 861 code
 * @param struct3d code for the 3D structure
 * @return detail if exists, 0xff otherwise.
 */
u8 api_Get3dStructDetail(hdmivsdb_t *vsdb, u8 vic, u8 struct3d);
/** Check if a certain video mode supporting a certain 3D structure has a detail
 * @param vsdb HDMI VSDB structure
 * @param vic video mode CEA 861 code
 * @param struct3d code for the 3D structure
...
/**
 * The observing the engine going through revocation states
 * @return a the state of the revocation the engine is going through
 * @note Used for debug purposes
 */
u8 api_HdcpRevocationState(void);
/**
 * HDCP engine extra observability information
...
/**
 * Write AN values to HDCP module
 * Either custom (64-bit value), or using the Random Number Generator
 * Interface if buffer is NULL
