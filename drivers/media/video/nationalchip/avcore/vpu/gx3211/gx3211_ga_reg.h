#ifndef __GX3211_GA_REG_H__
#define __GX3211_GA_REG_H__
...
typedef struct gx3211_ga_reg {
	...
	unsigned int    rBWF_SYNC_MODE;
	unsigned int    rNDSW_CHECK_EN;
	unsigned int    rReserved[2];
	unsigned int    rTimeOutCnt;
	unsigned int    rBusyStatus;
}Gx3211GaReg;
...
#define bGA_INTERRUPT_TIMEOUT	(16)
#define mGA_INTERRUPT_TIMEOUT	(0x1<<bGA_INTERRUPT_TIMEOUT)
#define bGA_INTERRUPT_TIMEOUT_READ	(17)
#define mGA_INTERRUPT_TIMEOUT_READ	(0x1<<bGA_INTERRUPT_TIMEOUT_READ)
...
#define GA_SET_INTERRUPT_TIMEOUT(reg) \
    REG_SET_BIT(&(reg), bGA_INTERRUPT_TIMEOUT)
#define GA_GET_INTERRUPT_TIMEOUT(reg) \
    REG_GET_BIT(&(reg), bGA_INTERRUPT_TIMEOUT_READ)
...
#define GA_SET_AS_ACCESS_ALIGN(reg)\
	REG_SET_BIT(&(reg), 0)

#define GA_CLR_AS_ACCESS_ALIGN(reg)\
	REG_CLR_BIT(&(reg), 0)
...
do {\
	...
	REG_SET_BIT(&(gx3211_clock_reg->cfg_mpeg_cold_reset), 5);\
	gx_mdelay(1);\
	REG_CLR_BIT(&(gx3211_clock_reg->cfg_mpeg_cold_reset), 5);\
}while(0)

#define START_GA(reg, job)\
	do {\
		/*GA_HOT_RESET();*/\
		GA_DISABLE              ( reg->rGA_ENABLE);\
		GA_SET_INTERRUPT_TYPE   ( reg->rGA_INTERRUPT);\
		GA_SET_INTERRUPT_DISABLE ( reg->rGA_INTERRUPT);\
		GA_SET_CMD_BUF_ADDR     ( reg->rCMD_BUFFER_ADDR, gx_virt_to_phys((unsigned int)job->cmd_start_addr));\
		GA_SET_CMD_BUF_LENGTH   ( reg->rCMD_BUFFER_LENGTH, job->max_cmd_num * GA_CMD_SIZE);\
