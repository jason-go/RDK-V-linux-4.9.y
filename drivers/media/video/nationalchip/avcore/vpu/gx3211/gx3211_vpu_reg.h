#ifndef __GX3211_VPU_REG_H__
#define __GX3211_VPU_REG_H__
...
typedef struct gx3211_vpu_reg {
	...
	unsigned int rRESERVED_2[6];//0x00C0~0x00D4

	unsigned int rMIX_CTRL1;
	...
	unsigned int rBUFF_CTRL2;//0x00F0
	...
	unsigned int rRESERVED_6[4];     //0x0170~0x017C

	unsigned int rMODULE_EDITION;//0x0180
	unsigned int rRESERVED_7[3]; //0x0184~0x018C

	unsigned int rSOSD_CTRL;//0x0190
	unsigned int rSOSD_FIRST_HEAD_PTR;
	unsigned int rSOSD_VIEW_SIZE;
	unsigned int rSOSD_ZOOM;
	unsigned int rSOSD_COLOR_KEY;//0x01A0
	unsigned int rSOSD_ALPHA_5551;
	unsigned int rSOSD_PHASE_0;
	unsigned int rSOSD_POSITION;
	unsigned int rSOSD_PARA;//0x01B0
	unsigned int rRESERVED_8[19];//0x01B4~ 0x01FC
	...
	unsigned int rRESERVED_9[448]; //0x0900~0x09FC
	unsigned int rPPGAMMA_COEF[5]; //0x1000~0x1010
	unsigned int rRESERVED_10[3];  //0x1014~0x101C
	unsigned int rPPCSC_CTRL[4]; //0x1020~0x102C
}Gx3211VpuReg;
...
#define mVPU_VPP_REQUEST_BLOCK (0x3FF<<bVPU_VPP_REQUEST_BLOCK)
 ...
 */
#define bVPU_OSD_RW_BYTESEQ_HIGH   (28)
...
#define bVPU_SPP_RW_BYTESEQ_HIGH   (26)
#define bVPU_SPP_RW_BYTESEQ_LOW    (18)
#define mVPU_SPP_RW_BYTESEQ_LOW    (0x3<<bVPU_SPP_RW_BYTESEQ_LOW)
#define VPU_SPP_SET_DATA_RW_BYTESEQ(reg, byte_seq)\
do\
{\
	REG_SET_FIELD(&(reg), mVPU_SPP_RW_BYTESEQ_LOW, byte_seq&0x3, bVPU_SPP_RW_BYTESEQ_LOW);\
	if(byte_seq>>2)\
		REG_SET_BIT(&(reg), bVPU_SPP_RW_BYTESEQ_HIGH);\
	else\
		REG_CLR_BIT(&(reg), bVPU_SPP_RW_BYTESEQ_HIGH);\
}while(0)
...
#define OSD_HEAD_SET_BIT(reg, bit) \
	do{\
		...
		ByteSequence byteseq = \
			((Gx3211VpuOsdPriv*)gx3211vpu_info->layer[GX_LAYER_OSD].priv)->regionhead_byte_seq;\
		switch(byteseq)\
		{\
		case ABCD_EFGH:\
