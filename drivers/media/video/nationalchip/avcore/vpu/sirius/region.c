...
extern struct osd_region** get_region_head_by_layer_id(GxLayerID layer_id);
...
static int region_set_colorkey(OsdRegion *region, GxColor color, int enable, GxLayerID layer_id)
{
	extern int osdx_color_key(GxLayerID layer_id, GxColor *color, int enable);

	region_set_colorkey_en(region, enable);
	return osdx_color_key(layer_id, &color, enable);
}
...
#else
#if REGION_POOL_EN
	SiriusVpuOsdPriv *priv = (SiriusVpuOsdPriv*)siriusvpu_info->layer[GX_LAYER_OSD].priv;
...
void region_free(OsdRegion *region)
{
	if (region) {
#if REGION_POOL_EN
		SiriusVpuOsdPriv *priv = (SiriusVpuOsdPriv*)siriusvpu_info->layer[GX_LAYER_OSD].priv;
...
void region_config(GxLayerID layer_id, OsdRegion *region, VpuSurface *surface, unsigned pos_x, unsigned pos_y)
{
	if (region && surface) {
		gx_memset(region, 0, sizeof(OsdRegion));
		region->pos_x   = pos_x;
...
void region_remove_all(GxLayerID layer_id)
{
	struct osd_region **p_region_head = get_region_head_by_layer_id(layer_id);
...
int region_submit(GxLayerID layer_id, OsdRegion *region)
{
	int ret = 0;
	struct osd_reg *reg = get_reg_by_layer_id(layer_id);
	struct osd_region **p_region_head = get_region_head_by_layer_id(layer_id);
	...
	if (*p_region_head == NULL) {
		*p_region_head = region;
		VPU_OSD_LIST_END_ENABLE(region->word7);
		VPU_OSD_SET_FIRST_HEAD(reg->rOSD_FIRST_HEAD_PTR, OSD_HEADPTR_PHYS(layer_id));
	} else {
...
OsdRegion* region_revoke(GxLayerID layer_id, VpuSurface *surface)
{
...
int sirius_region_add(GxLayerID layer_id, VpuSurface *surface, unsigned pos_x, unsigned pos_y)
{
	int ret = -1;
...
int sirius_region_reconfig(GxLayerID layer_id, OsdRegion *region, VpuSurface *surface, unsigned pos_x, unsigned pos_y)
{
	int ret = -1;
...
OsdRegion *sirius_region_revoke(GxLayerID layer_id, VpuSurface *surface)
{
	OsdRegion *region = NULL;
...
int sirius_region_remove(GxLayerID layer_id, VpuSurface *surface)
{
	int ret = -1;
	OsdRegion *region = NULL;
...
int sirius_region_update(GxLayerID layer_id, VpuSurface *surface, unsigned pos_x, unsigned pos_y)
{
	int ret = -1;
