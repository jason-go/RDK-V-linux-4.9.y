#ifndef __SIRIUS_SVPU_H__
#define __SIRIUS_SVPU_H__

#include "vpu_hal.h"
#include "gxav_bitops.h"
#include "gxav_vout_propertytypes.h"

//#define SVPU_REG_BASE_ADDR        (0x8a900000)
extern unsigned int SVPU_REG_BASE_ADDR;
...
typedef struct sirius_svpu_reg {
	...
	unsigned int rSCE_ZOOM_COEF;
	unsigned int rSCE_H_PHASE0;
	unsigned int rSCE_V_PHASE0;
	unsigned int rRESERVED_4[24];
...
#define bSVPU_AUTO_CAPMODE_EN  (12)
#define SVPU_SET_AUTO_CAPMODE_ENABLE(reg)\
	REG_SET_BIT(&(reg), bSVPU_AUTO_CAPMODE_EN);
#define SVPU_SET_AUTO_CAPMODE_DISABLE(reg)\
	REG_CLR_BIT(&(reg), bSVPU_AUTO_CAPMODE_EN);
