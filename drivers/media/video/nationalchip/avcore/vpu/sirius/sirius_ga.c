...
static GA  *siriusga = NULL;
static volatile SiriusGaReg *siriusga_reg = NULL;
...
static GaRet ga_job_submit(GaJob *job)
{
	...
	if(siriusga->mode == GX_GAMODE_BATCH) {
		if(job->cur_cmd_num > 0) {
			gx_dcache_clean_range(0,0);

			job->fin_cmd_num = 0;
			SUBMIT_JOB_TO_GA(siriusga_reg, job);
			gx_sem_wait(&(siriusga->job_fin_sem));
			GA_SET_INTERRUPT_CLR(siriusga_reg->rGA_INTERRUPT);
		...
		GA_SET_CMD_NUM ( siriusga_reg->rCMD_GAIN, 1);

		gx_interrupt_mask(GA_INT_SRC);
		GA_SEND_CMD_NUM_SIGNAL( siriusga_reg->rCMD_GAIN);
		REG_SET_BIT(&(siriusga_reg->rGA_INTERRUPT), bGA_INTERRUPT_CLR);
		REG_CLR_BIT(&(siriusga_reg->rGA_INTERRUPT), bGA_INTERRUPT_CLR);
		job->ready = 1;
		...
		while (addr == REG_GET_VAL(&siriusga_reg->rCMD_READ_ADDR));
	}
	ga_level--;
...
static GaRet ga_job_destroy(GaJob *job)
{
	...
	if(siriusga->mode == GX_GAMODE_FLOW) {
		unsigned int addr_cur, addr_start, size;

		addr_cur = REG_GET_VAL(&siriusga_reg->rCMD_READ_ADDR);
		...
		if (size != (job->max_cmd_num - 1) * GA_CMD_SIZE) {
			...
			GA_SET_CMD_BUF_LENGTH( siriusga_reg->rCMD_BUFFER_LENGTH, size + 2*GA_CMD_SIZE);
...
{
	if(siriusga) {
		siriusga->joblist.zt_coefficient = (unsigned int)gx_malloc(sizeof(ga_zt_coefficient));
		if(siriusga->joblist.zt_coefficient)
			gx_memcpy((void*)siriusga->joblist.zt_coefficient, (void*)ga_zt_coefficient, sizeof(ga_zt_coefficient));
		else
			gx_printf("malloc zt_coefficient failed!\n");

		siriusga->joblist.job_num = 0;
		gx_memset(siriusga->joblist.jobs, 0, sizeof(GaJob*)*GA_MAX_JOB_NUM);
	}
...
static GaRet ga_joblist_addjob(GaJob *job)
{
	unsigned int job_num = siriusga->joblist.job_num;

	if((job == NULL) || (siriusga->joblist.job_num + 1 > GA_MAX_JOB_NUM))
		return GA_RET_FAIL;

	siriusga->joblist.jobs[job_num] = job;
	siriusga->joblist.job_num ++;
...
static GaJob* ga_joblist_getcurrent(int create_if_null)
{
	unsigned int cur_job, job_num = siriusga->joblist.job_num;

	if(job_num > 0) {
		cur_job = job_num - 1;
		return siriusga->joblist.jobs[cur_job];
	}
...
static GaRet ga_joblist_removejob(GaJob **job)
{
	...
	cur_job = job_num - 1;
	if(job != NULL)
		*job =  siriusga->joblist.jobs[cur_job];

	siriusga->joblist.jobs[cur_job] = NULL;
	siriusga->joblist.job_num --;
...
static GaRet ga_joblist_destroy(void)
{
	unsigned int i, job_num = siriusga->joblist.job_num;

	for(i = 0; i < job_num; i++)
	{
		ga_job_destroy(siriusga->joblist.jobs[i]);
	}
	siriusga->joblist.job_num = 0;
	if(siriusga->joblist.zt_coefficient)
		gx_free((void*)siriusga->joblist.zt_coefficient);
...
static GaRet ga_submit_cmd(GaCmd *cmd)
{
	...
	gx_sem_wait(&siriusga->job_lock);

	ret = ga_submit_cmd_nolock(cmd);

	gx_sem_post(&siriusga->job_lock);
...
static GaRet ga_cmd_zoom(GaZoomParam *zoom)
{
	...
	cmd.Scaler.Scaler_Coefficent_Addr   = gx_virt_to_phys(siriusga->joblist.zt_coefficient);
...
GaRet siriusga_init()
{
	if(!gx_request_mem_region(GA_REG_BASE_ADDR, sizeof(SiriusGaReg)))
		GA_PRINTF("request_mem_region failed");

	siriusga_reg = (SiriusGaReg*)gx_ioremap(GA_REG_BASE_ADDR, sizeof(SiriusGaReg));

	gxav_clock_module_enable(MODULE_TYPE_GA, 1);
	if(gxav_firewall_access_align())
		GA_SET_AS_ACCESS_ALIGN(siriusga_reg->rNDSW_CHECK_EN);
	else
		GA_CLR_AS_ACCESS_ALIGN(siriusga_reg->rNDSW_CHECK_EN);
	GA_SET_CMD_LITTLE_ENDIAN(siriusga_reg->rCMD_ENDIANNESS);
	gxav_clock_module_enable(MODULE_TYPE_GA, 0);
	ga_clk_enable = 0;
...
GaRet siriusga_cleanup()
{
	if(siriusga_reg) {
		gx_iounmap(siriusga_reg);
		gx_release_mem_region(GA_REG_BASE_ADDR, sizeof(SiriusGaReg));
		siriusga_reg = NULL;
	}
	siriusga_close();
...
GaRet siriusga_open(void)
{
	siriusga = (GA*)gx_mallocz(sizeof(GA));
	IF_TRUE_RETURN_WITH_VAL(siriusga==NULL, GA_RET_FAIL);

	/* init ga */
	ga_joblist_init();
	gx_sem_create(&siriusga->job_lock, 1);
	gx_sem_create(&siriusga->job_fin_sem, 0);

	siriusga->mode = GX_GAMODE_BATCH;
	if(siriusga->mode == GX_GAMODE_FLOW) {
		GaJob *job = ga_job_create(3000);
		ga_joblist_addjob(job);
		START_GA(siriusga_reg, job);
	}

	return  GA_RET_OK;
}

GaRet siriusga_close(void)
{
	if(siriusga != NULL) {
		ga_joblist_destroy();
		gx_sem_delete(&siriusga->job_lock);
		gx_sem_delete(&siriusga->job_fin_sem);
		if(siriusga_reg) {
...
/* such big function */
GaRet siriusga_dfb_blit(GxVpuProperty_DfbBlit *property)
{
	return ga_dfb_blit(property, 1, &property->basic.srca.rect, &property->basic.dst.rect);
}

GaRet siriusga_batch_dfb_blit(GxVpuProperty_BatchDfbBlit *property)
{
	return ga_dfb_blit((GxVpuProperty_DfbBlit *)property, property->num, property->srcs, property->dsts);
}


GaRet siriusga_blit(GxVpuProperty_Blit *property)
{
	GaBlitParam param;
	GxBlitColorKeyInfo *sck, *tck;
	SiriusVpuSurface *sura, *surb, *dst;
	int ret = 0;
...
GaRet siriusga_fillrect(GxVpuProperty_FillRect *property)
{
	GaFillRectParam param;
	GxColorFormat color_format;
	SiriusVpuSurface *dst_surface = (SiriusVpuSurface*)property->surface;
...
GaRet siriusga_zoom(GxVpuProperty_ZoomSurface *property)
{
	GaZoomParam param;
	GxAvRect src_rect, dst_rect;
	SiriusVpuSurface *src = (SiriusVpuSurface*)property->src_surface;
	SiriusVpuSurface *dst = (SiriusVpuSurface*)property->dst_surface;
...
GaRet siriusga_turn(GxVpuProperty_TurnSurface *property)
{
	GaTurnParam param;
	unsigned int flip_mode[]   = {0,1,2};   /*no , v, h*/
...
GaRet siriusga_complet(GxVpuProperty_Complet *property)
{
	GaCompletParam param;
	GxAvRect src_rect, *dst_rect = &(property->dst_rect);
	SiriusVpuSurface src, *dst = (SiriusVpuSurface*)property->dst_surface;
...
static void gx_ga_end(void)
{
	...
	if(job) {
		if(siriusga->mode == GX_GAMODE_BATCH) {
			...
			if(job->ready) {
				job->ready = 0;
				while((REG_GET_VAL(&siriusga_reg->rGA_INTERRUPT) & 0x10) != 0x10);
			}
		}
	}
}

GaRet siriusga_setmode(GxVpuProperty_SetGAMode * property)
{
	GaJob *job = NULL;
	...
	if(siriusga->mode == property->mode)
		return GA_RET_OK;

	gx_sem_wait(&siriusga->job_lock);
	switch (siriusga->mode) {
		case GX_GAMODE_BATCH:
			while(siriusga->joblist.job_num)
				gx_ga_end();
	...
	siriusga->mode = property->mode;
	gx_sem_post(&siriusga->job_lock);

	return GA_RET_OK;
}

GaRet siriusga_wait(GxVpuProperty_WaitUpdate * property)
{
	if(siriusga->mode == GX_GAMODE_BATCH)
		return GA_RET_OK;
...
GaRet siriusga_begin(GxVpuProperty_BeginUpdate * property)
{
	GaRet ret;
	GaJob *job = NULL;
...
GaRet siriusga_end(GxVpuProperty_EndUpdate * property)
{
	gx_sem_wait(&siriusga->job_lock);
	gx_ga_end();
	gx_sem_post(&siriusga->job_lock);
...
GaRet siriusga_interrupt(int irq)
{
	GaJob *job = NULL;

	GA_CLEAN_INTERRUPT(siriusga_reg->rGA_INTERRUPT);
	...
	gx_sem_post(&(siriusga->job_fin_sem));
