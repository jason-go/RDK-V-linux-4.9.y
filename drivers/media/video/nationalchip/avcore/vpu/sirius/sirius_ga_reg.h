#ifndef __SIRIUS_GA_REG_H__
#define __SIRIUS_GA_REG_H__
...
//#define GA_REG_BASE_ADDR    (0x8A700000)
extern unsigned int GA_REG_BASE_ADDR;


typedef struct sirius_ga_reg {
...
#define GA_HOT_RESET()\
do {\
	extern volatile struct config_regs *sirius_config_reg;\
	REG_SET_BIT(&(sirius_config_reg->cfg_mpeg_hot_reset), 5);\
	gx_mdelay(1);\
	REG_CLR_BIT(&(sirius_config_reg->cfg_mpeg_hot_reset), 5);\
}while(0)
...
#define START_GA(reg, job)\
	do {\
		...
		GA_SET_CMD_BUF_ADDR     ( reg->rCMD_BUFFER_ADDR, gx_dma_to_phys((unsigned int)job->cmd_start_addr));\
...
{																		\
	...
	GX_CMD_SET_VAL(reg+0,                                             \
            ...
            GX_CMD_SET_VAL(reg+19,\
			       ...
			       REGION_VAL(0, 0x2, 1) | \
			       REGION_VAL(cmd->RGB888_EN, 0x1, 0));\
        }while(0)
