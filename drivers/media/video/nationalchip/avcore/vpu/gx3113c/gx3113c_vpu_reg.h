#ifndef __GX3113c_VPU_REG_H__
#define __GX3113c_VPU_REG_H__
...
typedef struct gx3113c_vpu_reg {
	...
	unsigned int rRESERVED_C[12]; //0x0104 108 10C ... 0x0130
	unsigned int rVIDEO_DAC_POWER; //0x0134
	unsigned int rVIDEO_SOUCRE_SEL; //0x0138
	unsigned int rRESERVED_D[49]; //0x013c 140 144 ... 0x01FC
...
#define VPU_PP_SET_FILTER_SIGN(reg, value) \
...
#define VPU_SET_DATA_RW_BYTESEQ(reg, byte_seq)\
do\
{\
...
#define bVPU_REGIONHEAD_BYTESEQ		(12)
#define mVPU_REGIONHEAD_BYTESEQ		(0x7<<bVPU_REGIONHEAD_BYTESEQ)
#define VPU_SET_REGIONHEAD_BYTESEQ(reg, byte_seq)\
do\
{\
	REG_SET_FIELD(&(reg), mVPU_REGIONHEAD_BYTESEQ, byte_seq&0x7, bVPU_REGIONHEAD_BYTESEQ);\
}while(0)
...
#define OSD_HEAD_SET_BIT(reg, bit) \
	do{\
		...
		ByteSequence byteseq = \
			((Gx3113cVpuOsdPriv*)gx3113cvpu_info->layer[GX_LAYER_OSD].priv)->regionhead_byte_seq;\
		switch(byteseq)\
		{\
		case ABCD_EFGH:\
