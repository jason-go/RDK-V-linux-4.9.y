...
static GA  *gx3113cga = NULL;
static volatile Gx3113cGaReg *gx3113cga_reg = NULL;
...
static GaRet ga_job_submit(GaJob *job)
{
	...
	if(job->cur_cmd_num > 0) {
		...
		job->fin_cmd_num = 0;
		SUBMIT_JOB_TO_GA(gx3113cga_reg, job);
		gx_sem_wait(&(gx3113cga->job_fin_sem));
		GA_SET_INTERRUPT_CLR(gx3113cga_reg->rGA_INTERRUPT);
...
{
	if(gx3113cga) {
		gx3113cga->joblist.zt_coefficient = (unsigned int)gx_malloc(sizeof(ga_zt_coefficient));
		if(gx3113cga->joblist.zt_coefficient)
			gx_memcpy((void*)gx3113cga->joblist.zt_coefficient, (void*)ga_zt_coefficient, sizeof(ga_zt_coefficient));
		else
			gx_printf("malloc zt_coefficient failed!\n");

		gx3113cga->joblist.job_num = 0;
		gx_memset(gx3113cga->joblist.jobs, 0, sizeof(GaJob*)*GA_MAX_JOB_NUM);
	}
...
static GaRet ga_joblist_addjob(GaJob *job)
{
	unsigned int job_num = gx3113cga->joblist.job_num;

	if((job == NULL) || (gx3113cga->joblist.job_num + 1 > GA_MAX_JOB_NUM))
		return GA_RET_FAIL;

	gx3113cga->joblist.jobs[job_num] = job;
	gx3113cga->joblist.job_num ++;
...
static GaJob* ga_joblist_getcurrent(int create_if_null)
{
	unsigned int cur_job, job_num = gx3113cga->joblist.job_num;

	if(job_num > 0) {
		cur_job = job_num - 1;
		return gx3113cga->joblist.jobs[cur_job];
	}
...
static GaRet ga_joblist_removejob(GaJob **job)
{
	...
	cur_job = job_num - 1;
	if(job != NULL)
		*job =  gx3113cga->joblist.jobs[cur_job];

	gx3113cga->joblist.jobs[cur_job] = NULL;
	gx3113cga->joblist.job_num --;
...
static GaRet ga_joblist_destroy(void)
{
	unsigned int i, job_num = gx3113cga->joblist.job_num;

	for(i = 0; i < job_num; i++)
	{
		ga_job_destroy(gx3113cga->joblist.jobs[i]);
	}
	gx3113cga->joblist.job_num = 0;
	if(gx3113cga->joblist.zt_coefficient)
		gx_free((void*)gx3113cga->joblist.zt_coefficient);
...
static GaRet ga_submit_cmd(GaCmd *cmd)
{
	...
	gx_sem_wait(&gx3113cga->job_lock);
	create_if_null = 1;
	...
	gx_sem_post(&gx3113cga->job_lock);
...
static GaRet ga_cmd_zoom(GaZoomParam *zoom)
{
	...
	cmd.Scaler.Scaler_Coefficent_Addr   = gx_virt_to_phys(gx3113cga->joblist.zt_coefficient);
...
GaRet gx3113cga_init()
{
	if(!gx_request_mem_region(GA_REG_BASE_ADDR, sizeof(Gx3113cGaReg)))
		GA_PRINTF("request_mem_region failed");

	gx3113cga_reg = (Gx3113cGaReg*)gx_ioremap(GA_REG_BASE_ADDR, sizeof(Gx3113cGaReg));

	GA_SET_CMD_LITTLE_ENDIAN(gx3113cga_reg->rCMD_ENDIANNESS);

	return GA_RET_OK;
}

GaRet gx3113cga_cleanup()
{
	if(gx3113cga_reg) {
		gx_iounmap(gx3113cga_reg);
		gx_release_mem_region(GA_REG_BASE_ADDR, sizeof(Gx3113cGaReg));
		gx3113cga_reg = NULL;
	}
	gx3113cga_close();
...
GaRet gx3113cga_open(void)
{
	gx3113cga = (GA*)gx_malloc(sizeof(GA));
	IF_TRUE_RETURN_WITH_VAL(gx3113cga==NULL, GA_RET_FAIL);

	/* init ga */
	ga_joblist_init();
	gx_sem_create(&gx3113cga->job_lock, 1);
	gx_sem_create(&gx3113cga->job_fin_sem, 0);

	return  GA_RET_OK;
}

GaRet gx3113cga_close(void)
{
	if(gx3113cga != NULL) {
		ga_joblist_destroy();
		gx_sem_delete(&gx3113cga->job_lock);
		gx_sem_delete(&gx3113cga->job_fin_sem);
		if(gx3113cga_reg) {
...
/* such big function */
GaRet gx3113cga_dfb_blit(GxVpuProperty_DfbBlit *property)
{
...
GaRet gx3113cga_blit(GxVpuProperty_Blit *property)
{
	GaBlitParam param;
	GxBlitColorKeyInfo *sck, *tck;
	Gx3113cVpuSurface *sura, *surb, *dst;
...
GaRet gx3113cga_fillrect(GxVpuProperty_FillRect *property)
{
	GaFillRectParam param;
	GxColorFormat color_format;
	Gx3113cVpuSurface *dst_surface = (Gx3113cVpuSurface*)property->surface;
...
GaRet gx3113cga_zoom(GxVpuProperty_ZoomSurface *property)
{
	GaZoomParam param;
	GxAvRect src_rect, dst_rect;
	Gx3113cVpuSurface *src = (Gx3113cVpuSurface*)property->src_surface;
	Gx3113cVpuSurface *dst = (Gx3113cVpuSurface*)property->dst_surface;
...
GaRet gx3113cga_turn(GxVpuProperty_TurnSurface *property)
{
	GaTurnParam param;
	unsigned int flip_mode[]   = {0,1,2};   /*no , v, h*/
...
GaRet gx3113cga_complet(GxVpuProperty_Complet *property)
{
	GaCompletParam param;
	GxAvRect src_rect, *dst_rect = &(property->dst_rect);
	Gx3113cVpuSurface src, *dst = (Gx3113cVpuSurface*)property->dst_surface;
...
GaRet gx3113cga_begin(GxVpuProperty_BeginUpdate * property)
{
	GaRet ret;
	GaJob *job = NULL;
...
GaRet gx3113cga_end(GxVpuProperty_EndUpdate * property)
{
	GaJob *job = NULL;
...
GaRet gx3113cga_interrupt(int irq)
{
	GaJob *job = NULL;

	GA_CLEAN_INTERRUPT(gx3113cga_reg->rGA_INTERRUPT);
	...
	gx_sem_post(&(gx3113cga->job_fin_sem));
